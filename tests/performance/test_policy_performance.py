...
# Size knobs for the benchmark fixtures below. Deliberately independent of
# each other: the number of distinct token ranges (routing keys) the mock
# cluster's replica map knows about, and the number of queries generated
# against it, are two separate concerns and can be tuned independently
# without touching the other fixture.
TOKEN_RANGE_COUNT = 100_000
QUERY_COUNT = 100_000
...
def vnode_cluster():
    ...
    # Pre-calculate replica assignments for TOKEN_RANGE_COUNT routing keys.
    # This is independent of how many queries are later generated against
    # this cluster (see benchmark_queries below).
    for i in range(TOKEN_RANGE_COUNT):
        key = struct.pack(">I", i)
...
def benchmark_queries():
    """Generate QUERY_COUNT deterministic mock queries.

    Sized independently of vnode_cluster's TOKEN_RANGE_COUNT: the query load
    and the cluster's token-range cardinality are separate fixtures/knobs, so
    either can be changed without touching the other. Routing keys wrap
    around modulo TOKEN_RANGE_COUNT so this still works if QUERY_COUNT is
    changed to exceed TOKEN_RANGE_COUNT (multiple queries landing in the same
    token range, as would happen on a real cluster).
    """
    return [
        MockStatement(routing_key=struct.pack(">I", i % TOKEN_RANGE_COUNT))
