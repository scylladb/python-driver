...
class WriteQueryParamsBufferAccumulationTest(unittest.TestCase):
    ...
    def test_batch_send_body_single_write_call(self):
        """send_body must emit exactly one f.write() call, not one per field."""
        queries = [
        ...
        counting_io = Mock(wraps=io.BytesIO())
        msg.send_body(counting_io, protocol_version=5)
        self.assertEqual(counting_io.write.call_count, 1)
