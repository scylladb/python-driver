...
class _CountingWriter:
    """Write-only fake stream that counts write() calls (not a BytesIO)."""
...
class WriteQueryParamsBufferAccumulationTest(unittest.TestCase):
    ...
    def test_batch_send_body_single_write_call_prepared_params(self):
        """Prepared-statement params must emit exactly one f.write() call."""
        queries = [(True, b'\x01\x02', [b'val1', None, _UNSET_VALUE])]
        msg = BatchMessage(batch_type=BatchType.LOGGED, queries=queries,
                           consistency_level=1)
        w = _CountingWriter()
        msg.send_body(w, protocol_version=4)
        self.assertEqual(w.calls, 1)

    def test_batch_send_body_single_write_call_unprepared_params(self):
        """Unprepared (string query) params must emit exactly one f.write() call."""
        queries = [
            (False, 'SELECT 1', [b'a', b'b']),
            (False, 'SELECT 2', [None]),
        ]
        msg = BatchMessage(batch_type=BatchType.LOGGED, queries=queries,
                           consistency_level=1)
    ...
    def test_batch_send_body_single_write_call_with_trailer_fields(self):
        """Optional trailer fields (serial consistency, timestamp, keyspace) still yield one write()."""
        queries = [
            (True, b'\x01\x02', [b'val1', None]),
            (False, 'SELECT 1', [_UNSET_VALUE]),
        ]
        msg = BatchMessage(batch_type=BatchType.LOGGED, queries=queries,
                           consistency_level=1, serial_consistency_level=8,
                           timestamp=12345, keyspace='ks')
        w = _CountingWriter()
        msg.send_body(w, protocol_version=5)
        self.assertEqual(w.calls, 1)

    def test_batch_send_body_matches_reference_bytes(self):
        """Accumulated send_body output must equal the wire format, byte for byte."""
        queries = [
            (True, b'\x01\x02\x03\x04', [b'val1', None]),
            (False, 'INSERT INTO t (k) VALUES (?)', [b'\x02', _UNSET_VALUE]),
            (True, b'\xaa\xbb\xcc\xdd', [b'']),
        ]
        msg = BatchMessage(batch_type=BatchType.LOGGED, queries=queries,
                           consistency_level=1, serial_consistency_level=8,
                           timestamp=12345, keyspace='ks')
        buf = io.BytesIO()
        msg.send_body(buf, protocol_version=5)

        expected = struct.pack('>B', BatchType.LOGGED.value) + struct.pack('>H', len(queries))
        # query 1: prepared, one value and one NULL param
        expected += struct.pack('>B', 1) + struct.pack('>H', len(queries[0][1])) + queries[0][1]
        expected += struct.pack('>H', 2)
        expected += struct.pack('>i', len(b'val1')) + b'val1'
        expected += struct.pack('>i', -1)
        # query 2: unprepared, one value and one UNSET param
        stmt = queries[1][1].encode('utf8')
        expected += struct.pack('>B', 0) + struct.pack('>i', len(stmt)) + stmt
        expected += struct.pack('>H', 2)
        expected += struct.pack('>i', 1) + b'\x02'
        expected += struct.pack('>i', -2)
        # query 3: prepared, one empty-bytes param
        expected += struct.pack('>B', 1) + struct.pack('>H', len(queries[2][1])) + queries[2][1]
        expected += struct.pack('>H', 1) + struct.pack('>i', 0)
        # consistency level, int flags (protocol v5), serial consistency, timestamp, keyspace
        expected += struct.pack('>H', 1)
        expected += struct.pack('>i', 0x10 | 0x20 | 0x80)
        expected += struct.pack('>H', 8)
        expected += struct.pack('>q', 12345)
        ks = b'ks'
        expected += struct.pack('>H', len(ks)) + ks
