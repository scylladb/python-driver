...
from cassandra.protocol import (
    ...
    _UNSET_VALUE, write_value, write_string, write_consistency_level, write_byte
)
...
class WriteQueryParamsBufferAccumulationTest(unittest.TestCase):
    ...
    def test_normal_params(self):
        ...
        self.assertTrue(raw.endswith(expected))
    ...
    def test_none_query_params(self):
        """When query_params is None, no param block or _VALUES_FLAG is written."""
        msg1 = ExecuteMessage(query_id=b'qid', query_params=None,
                              consistency_level=1)
        raw1 = self._execute_msg_bytes(msg1, protocol_version=4)
        expected1 = io.BytesIO()
        write_string(expected1, b'qid')
        write_consistency_level(expected1, 1)
        write_byte(expected1, 0)  # flags: no _VALUES_FLAG, no param block follows
        self.assertEqual(expected1.getvalue(), raw1)
    ...
    def test_query_message_with_params(self):
        ...
        self.assertTrue(raw.getvalue().endswith(expected))
    ...
    def test_proto_v3_vs_v4_params(self):
        ...
        expected = self._reference_write_value_bytes(params)
        self.assertTrue(raw_v3.endswith(expected))
        self.assertTrue(raw_v4.endswith(expected))
    ...
    def test_encode_message_roundtrip(self):
        ...
        # The frame body ends with the param bytes (no trailing fields set)
        expected_param_bytes = self._reference_write_value_bytes(params)
        # frame may be memoryview/bytearray; convert to bytes for endswith
        frame_bytes = bytes(frame)
        self.assertTrue(frame_bytes.endswith(expected_param_bytes))
