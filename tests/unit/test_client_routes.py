...
class TestClientRoutesHandler(unittest.TestCase):
    ...
    @patch('cassandra.client_routes.time.sleep')
    @patch.object(_ClientRoutesHandler, '_query_routes')
    def test_reconnect_retries_three_times_then_gives_up(self, mock_query, mock_sleep):
        """handle_control_connection_reconnect should retry 3 times on failure."""
        handler = _ClientRoutesHandler(self.config)
        mock_cc = Mock()
        mock_query.side_effect = Exception("transient error")

        handler.handle_control_connection_reconnect(mock_cc)

        self.assertEqual(mock_query.call_count, 3)
        # sleep(1) called between attempts 1→2 and 2→3
        self.assertEqual(mock_sleep.call_count, 2)

    @patch('cassandra.client_routes.time.sleep')
    @patch.object(_ClientRoutesHandler, '_query_routes')
    def test_reconnect_succeeds_on_second_attempt(self, mock_query, mock_sleep):
        """handle_control_connection_reconnect should stop retrying after success."""
        handler = _ClientRoutesHandler(self.config)
        mock_cc = Mock()
        host_id = uuid.uuid4()
        ...
        mock_query.side_effect = [
            ...
            [_Route(connection_id=str(self.conn_id), host_id=host_id, address="ok.com", port=9042)],
        ]
...
class TestClientRoutesEndPoint(unittest.TestCase):
    ...
    def test_resolve_host_missing_port_raises(self):
        """resolve_host should raise AttributeError when route has no port."""
        host_id = uuid.uuid4()
        self.handler._routes.update([
            _Route(connection_id=str(self.conn_id), host_id=host_id,
...
class TestClientRoutesEndPointFactory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.handler = _ClientRoutesHandler(self.config, ssl_enabled=False)
        self.factory = ClientRoutesEndPointFactory(self.handler, port=9042)
