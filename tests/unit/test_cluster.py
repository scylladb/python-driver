...
from cassandra.connection import ConnectionBusy, ConnectionException, SSLSessionCache
...
class TestSSLSessionCacheAutoCreation(unittest.TestCase):
    ...
    def test_no_cache_when_only_ssl_options_set(self):
    ...
    def test_no_auto_cache_on_asyncio_reactor(self):
        # The asyncio reactor cannot restore/store TLS sessions, so a cache
        # must not be auto-created for it (an explicit cache is still honored).
        import ssl
        try:
            from cassandra.io.asyncioreactor import AsyncioConnection
        except Exception:
            raise unittest.SkipTest("asyncio reactor not available")
        ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        cluster = Cluster(contact_points=['127.0.0.1'], ssl_context=ctx,
        ...
        # But an explicitly-provided cache is still used.
        custom = SSLSessionCache()
        cluster = Cluster(contact_points=['127.0.0.1'], ssl_context=ctx,
                          connection_class=AsyncioConnection,
                          ssl_session_cache=custom)
