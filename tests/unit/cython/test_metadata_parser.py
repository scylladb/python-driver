...
class MetadataParserTest(unittest.TestCase):
    ...
    @cythontest
    def test_oversized_colcount_rejected(self):
        """
        colcount is peer-controlled: a truncated/malformed frame claiming an
        enormous column count (e.g. 0x7fffffff) must be rejected before
        `[None] * colcount` allocates, instead of amplifying a small wire
        payload into a multi-GB allocation (CWE-400).
        """
        recv = self._make_recv()
        flags = 0x0001  # _FLAGS_GLOBAL_TABLES_SPEC
        body = (int32_pack(flags) + int32_pack(0x7fffffff)
