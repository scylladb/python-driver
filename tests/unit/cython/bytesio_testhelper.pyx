...
def test_seek_out_of_range():
    # seek() must reject offsets outside [0, size] -- a negative or
    # past-end pos would let a later read() return bytes outside the buffer.
    cdef BytesIOReader reader = BytesIOReader(b'abcdef')
    with pytest.raises(ValueError):
        reader.seek(-1)
    with pytest.raises(ValueError):
        reader.seek(7)
    reader.seek(6)  # exactly at EOF is still valid
