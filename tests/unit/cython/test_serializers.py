...
class TestSerVectorTypeGenericFallback(unittest.TestCase):
    ...
    def test_smallint_vector_is_variable_length(self):
        """ShortType (smallint) is not one of SerVectorType's fixed-width
        fast-path subtypes (float/double/int32), so it must take the
        generic fallback, which defers to ``subtype.serial_size()``.

        ShortType.serial_size() correctly returns None (no override in
        cqltypes.py), matching real Cassandra wire behavior: smallint
        vector elements are vint-length-prefixed, not fixed 2-byte values
        (AbstractType.valueLengthIfFixed() / ShortType.java do not
        override the variable-length default). This test locks in that
        the Cython path does not regress to treating smallint as
        fixed-width.
        """
        vec_type = _make_vector_type(ShortType, 4)
        self.assertIsNone(vec_type.subtype.serial_size())
        ser = SerVectorType(vec_type)
        values = [3, 2, 41, 12]
        cython_bytes = ser.serialize(values, PROTO)
        python_bytes = vec_type.serialize(values, PROTO)
        self.assertEqual(cython_bytes, python_bytes)
        # A (incorrect) fixed-2-byte-per-element encoding would be exactly
        # 8 bytes; the correct vint-prefixed encoding must not collapse to
        # that size (each element gets a 1-byte vint length prefix too).
        self.assertNotEqual(len(cython_bytes), 4 * 2)
        deserialized = vec_type.deserialize(cython_bytes, PROTO)
...
@cythontest
class TestSerVectorTypeZeroDimensionBuffer(unittest.TestCase):
    """Zero-dimension vectors must not dereference an empty buffer view.

    Each buffer fast-path (``_serialize_float_buffer``, ``_serialize_double_buffer``,
    ``_serialize_int32_buffer``) special-cases ``buf_size == 0`` before the
    big-endian-machine ``memcpy(..., &view[0], ...)`` call, since indexing an
    empty typed memoryview at ``[0]`` would be invalid. Cover all three
    buffer-fast-path dtypes with empty NumPy arrays as a regression guard.
    """

    @numpytest
    def test_float_zero_dim_numpy(self):
        import numpy as np
        vec_type = _make_vector_type(FloatType, 0)
        ser = SerVectorType(vec_type)
        values = np.asarray([], dtype=np.float32)
        cython_bytes = ser.serialize(values, PROTO)
        self.assertEqual(cython_bytes, b"")

    @numpytest
    def test_double_zero_dim_numpy(self):
        import numpy as np
        vec_type = _make_vector_type(DoubleType, 0)
        ser = SerVectorType(vec_type)
    ...
    @numpytest
    def test_int32_zero_dim_numpy(self):
        import numpy as np
        vec_type = _make_vector_type(Int32Type, 0)
        ser = SerVectorType(vec_type)
