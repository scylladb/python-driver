...
class TestGetConfig(unittest.TestCase):
    def test_invalid_object(self):
        ...
        # no default
        # ... as a policy
        assert insights_registry.serialize(obj, policy=True) == {
            "type": "NoConfAsDict",
            "namespace": ns,
            "options": {},
        }
        # ... not as a policy (default)
        assert insights_registry.serialize(obj) == {
            "type": "NoConfAsDict",
            "namespace": ns,
        }
        # with default
        assert (
            insights_registry.serialize(obj, default=sentinel.attr_err_default)
            is sentinel.attr_err_default
        )
    ...
    def test_successful_return(self):
        ...
        assert (
            insights_registry.serialize(SuperclassSentinel())
            is sentinel.serialized_superclass
        )
        assert (
            insights_registry.serialize(SubclassSentinel())
            is sentinel.serialized_superclass
        )

        # with default -- same behavior
        assert (
            insights_registry.serialize(SubclassSentinel(), default=object())
            is sentinel.serialized_superclass
        )
...
class TestConfigAsDict(unittest.TestCase):
    ...
    def test_graph_options(self):
        ...
        assert insights_registry.serialize(go) == {
    ...
    def test_execution_profile(self):
        self.maxDiff = None
        assert insights_registry.serialize(ExecutionProfile()) == {
    ...
    def test_graph_execution_profile(self):
        self.maxDiff = None
        assert insights_registry.serialize(GraphExecutionProfile()) == {
            "consistency": "LOCAL_ONE",
            "continuousPagingOptions": None,
            "loadBalancing": {
                "namespace": "cassandra.policies",
    ...
    def test_graph_analytics_execution_profile(self):
        self.maxDiff = None
        assert insights_registry.serialize(GraphAnalyticsExecutionProfile()) == {
            "consistency": "LOCAL_ONE",
            "continuousPagingOptions": None,
            "loadBalancing": {
                "namespace": "cassandra.policies",
    ...
    def test_DC_aware_round_robin_policy(self):
        assert insights_registry.serialize(DCAwareRoundRobinPolicy()) == {
            "namespace": "cassandra.policies",
            "options": {"local_dc": "", "used_hosts_per_remote_dc": 0},
            "type": "DCAwareRoundRobinPolicy",
        }
        assert insights_registry.serialize(
            DCAwareRoundRobinPolicy(
                local_dc="fake_local_dc", used_hosts_per_remote_dc=15
            )
        ) == {
            "namespace": "cassandra.policies",
    ...
    def test_token_aware_policy(self):
        assert insights_registry.serialize(
            TokenAwarePolicy(child_policy=LoadBalancingPolicy())
        ) == {
            "namespace": "cassandra.policies",
            "options": {
                "child_policy": {
    ...
    def test_whitelist_round_robin_policy(self):
        assert insights_registry.serialize(
            WhiteListRoundRobinPolicy(["127.0.0.3"])
        ) == {
            "namespace": "cassandra.policies",
    ...
    def test_host_filter_policy(self):
        ...
        assert insights_registry.serialize(
            HostFilterPolicy(LoadBalancingPolicy(), my_predicate)
        ) == {
            "namespace": "cassandra.policies",
            "options": {
                "child_policy": {
    ...
    def test_constant_reconnection_policy(self):
        assert insights_registry.serialize(ConstantReconnectionPolicy(3, 200)) == {
    ...
    def test_exponential_reconnection_policy(self):
        assert insights_registry.serialize(
            ExponentialReconnectionPolicy(4, 100, 10)
        ) == {
    ...
    def test_retry_policy(self):
        assert insights_registry.serialize(RetryPolicy()) == {
            "type": "RetryPolicy",
            "namespace": "cassandra.policies",
            "options": {},
        }
    ...
    def test_spec_exec_policy(self):
        assert insights_registry.serialize(SpeculativeExecutionPolicy()) == {
    ...
    def test_constant_spec_exec_policy(self):
        assert insights_registry.serialize(
            ConstantSpeculativeExecutionPolicy(100, 101)
        ) == {
            "type": "ConstantSpeculativeExecutionPolicy",
            "namespace": "cassandra.policies",
            "options": {"delay": 100, "max_attempts": 101},
        }

    def test_wrapper_policy(self):
        assert insights_registry.serialize(WrapperPolicy(LoadBalancingPolicy())) == {
            "namespace": "cassandra.policies",
            "options": {
                "child_policy": {
                    "namespace": "cassandra.policies",
