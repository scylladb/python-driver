...
class LWTRetryPolicyTest(unittest.TestCase):
    ...
    def _make_policy(self, max_num_retries=3):
        return LWTRetryPolicy(max_num_retries=max_num_retries)
    ...
    def test_cas_write_timeout_retries_same_host(self):
        ...
        self.assertEqual(consistency, ConsistencyLevel.QUORUM)
    ...
    def test_cas_write_timeout_retries_with_backoff(self):
        ...
        policy = self._make_policy(max_num_retries=5)
        delays = []
        ...
        # Jitter is ± min_interval/2 = ±0.05s.
        # delay_0 base=0.1s (range 0.05–0.15), delay_2 base=0.4s (range 0.35–0.45).
        # The gap is wide enough that delay_0 < delay_2 always holds.
    ...
    def test_cas_write_timeout_max_retries_exceeded(self):
        ...
        policy = self._make_policy(max_num_retries=2)
        retry, consistency, delay = policy.on_write_timeout(
            query=None, consistency=ConsistencyLevel.QUORUM,
            write_type=WriteType.CAS,
    ...
    def test_cas_write_timeout_preserves_consistency(self):
        ...
        for cl in [ConsistencyLevel.QUORUM, ConsistencyLevel.LOCAL_QUORUM,
            ...
            retry, consistency, _ = policy.on_write_timeout(
                ...
                required_responses=3, received_responses=1, retry_num=0)
            self.assertEqual(retry, RetryPolicy.RETRY)
            self.assertEqual(consistency, cl, f"Expected {cl}, got {consistency}")
    ...
    def test_batch_log_write_timeout_max_retries_exceeded(self):
        """BATCH_LOG write timeout should RETHROW when max retries exceeded."""
        policy = self._make_policy(max_num_retries=1)
        retry, consistency, delay = policy.on_write_timeout(
            query=None, consistency=ConsistencyLevel.QUORUM,
            write_type=WriteType.BATCH_LOG,
            required_responses=3, received_responses=1, retry_num=1)
        self.assertEqual(retry, RetryPolicy.RETHROW)
    ...
    def test_serial_read_timeout_retries_same_host(self):
        ...
        retry, consistency, delay = policy.on_read_timeout(
            ...
            data_retrieved=False, retry_num=0)
        self.assertEqual(retry, RetryPolicy.RETRY)
        self.assertEqual(consistency, ConsistencyLevel.SERIAL)
        self.assertIsNotNone(delay)
        self.assertGreater(delay, 0)
    ...
    def test_local_serial_read_timeout_retries_same_host(self):
        ...
        retry, consistency, delay = policy.on_read_timeout(
            ...
            data_retrieved=False, retry_num=0)
        self.assertEqual(retry, RetryPolicy.RETRY)
        self.assertEqual(consistency, ConsistencyLevel.LOCAL_SERIAL)
        self.assertIsNotNone(delay)
        self.assertGreater(delay, 0)
    ...
    def test_default_constructor(self):
        ...
        self.assertEqual(policy.max_num_retries, 3)
        self.assertEqual(policy.min_interval, 0.1)
        self.assertEqual(policy.max_interval, 10.0)
    ...
    def test_custom_constructor(self):
        ...
        self.assertEqual(policy.max_num_retries, 5)
        self.assertEqual(policy.min_interval, 0.5)
        self.assertEqual(policy.max_interval, 30.0)
    ...
    def test_all_methods_return_3_tuples(self):
        ...
        result = policy.on_write_timeout(
            ...
            write_type=WriteType.CAS,
            required_responses=3, received_responses=1, retry_num=0)
        self.assertEqual(len(result), 3, f"Expected 3-tuple, got {result}")
