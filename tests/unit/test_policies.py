...
class TokenAwarePolicyTest(unittest.TestCase):
    ...
    def test_tablet_found_but_no_matching_replicas_does_not_fall_back_to_vnode(self):
        """A tablet found with no replicas present in the child plan must not
        fall back to vnode replicas -- the tablet route was resolved, just to
        an empty set (e.g. none of its replica host_ids are in the live plan)."""
        hosts = [Host(DefaultEndPoint(str(i)), SimpleConvictionPolicy, host_id=uuid.uuid4()) for i in range(4)]
        for host in hosts:
            host.set_up()
        ...
        cluster.metadata = Mock(spec=Metadata)
        cluster.metadata._tablets = Mock(spec=Tablets)
        # Tablet's replicas are host_ids not present in the child's plan.
        cluster.metadata._tablets.get_tablet_for_key.return_value = Tablet(replicas=[(uuid.uuid4(), 0)])
        cluster.metadata._tablets.table_has_tablets.return_value = True
        cluster.metadata.token_map = Mock()
        cluster.metadata.token_map.token_class.from_key.side_effect = lambda key: key
        cluster.metadata.token_map.get_replicas.return_value = hosts
        cluster.metadata.token_map.tokens_to_hosts_by_ks = {'ks': {}}
        ...
        # The tablet was found, so the vnode fallback must not have kicked in.
        assert cluster.metadata.token_map.get_replicas.call_count == 0
        # No replicas matched the tablet, so the plan falls back to the child
        # policy's un-prioritized hosts, not the vnode replica set.
        assert set(plan) == set(hosts)
