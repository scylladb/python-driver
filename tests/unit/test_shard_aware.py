...
from cassandra.cluster import ShardAwareOptions, NoDelayShardReconnectionScheduler, ShardReconnectionScheduler, \
    _Scheduler
from cassandra.policies import NoDelayReconnectionPolicy, ShardReconnectionScopeHost, ConstantReconnectionPolicy
...
class TestShardAware(unittest.TestCase):
    ...
    def test_advanced_shard_aware_port_no_delay(self):
        # with NoDelayReconnectionPolicy all the connections should be created right away
        self._run_test_advanced_shard_aware_port(4, NoDelayReconnectionPolicy(), 4, 4)

    def test_advanced_shard_aware_port_delay(self):
        # with ConstantReconnectionPolicy first connection is created right away, others are delayed
        self._run_test_advanced_shard_aware_port(4, ConstantReconnectionPolicy(1), 1, 4)

    def _run_test_advanced_shard_aware_port(self, shard_count, shard_reconnection_policy, expected_count, expected_after):
        """
        ...
        class MockSession(MagicMock):
            ...
            def __init__(self, is_ssl=False, *args, **kwargs):
                ...
                if isinstance(shard_reconnection_policy, NoDelayReconnectionPolicy):
                    ...
                    self.shard_reconnection_scheduler = ShardReconnectionScheduler(
                        self, self.cluster, ShardReconnectionScopeHost(), shard_reconnection_policy)
                self.futures = []
        ...
        try:
            for port, is_ssl in [(19042, False), (19045, True)]:
                ...
                sleep_time = 0
                schedule = shard_reconnection_policy.new_schedule()
