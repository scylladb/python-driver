...
class StrategiesTest(unittest.TestCase):
    def test_replication_strategy(self):
        ...
        assert rs.create("OldNetworkTopologyStrategy", None) == _UnknownStrategy(
            "OldNetworkTopologyStrategy", None
        )
        ...
        assert rs.create("xxxxxxxx", fake_options_map) == _UnknownStrategy(
            "xxxxxxxx", fake_options_map
        )
    ...
    def test_simple_replication_type_parsing(self):
        ...
        hosts = [
            Host("dc1.{}".format(host), SimpleConvictionPolicy, host_id=uuid.uuid4())
            for host in range(3)
        ]
        token_to_host = dict(zip(ring, hosts))
        assert simple_int.make_token_replica_map(
            token_to_host, ring
    ...
    def test_transient_replication_parsing(self):
        ...
        token_to_host = dict(zip(ring, hosts))
        assert simple_transient.make_token_replica_map(
            token_to_host, ring
        ) == simple_str.make_token_replica_map(token_to_host, ring)
    ...
    def test_nts_replication_parsing(self):
        ...
        token_to_host = dict(zip(ring, hosts))
        assert nts_int.make_token_replica_map(
            token_to_host, ring
    ...
    def test_nts_transient_parsing(self):
        ...
        assert nts_transient.dc_replication_factors_info["dc1"] == ReplicationFactor(
            3, 1
        )
        assert nts_transient.dc_replication_factors_info["dc2"] == ReplicationFactor(
            5, 1
        )
        ...
        token_to_host = dict(zip(ring, hosts))
        assert nts_transient.make_token_replica_map(
            token_to_host, ring
        ) == nts_str.make_token_replica_map(token_to_host, ring)
    ...
    def test_nts_token_performance(self):
        ...
        for i in range(dc1hostnum):
            host = Host(
                "dc1.{0}".format(i), SimpleConvictionPolicy, host_id=uuid.uuid4()
            )
            host.set_location_info("dc1", "rack1")
    ...
    def test_ss_equals(self):
        assert NetworkTopologyStrategy({"dc1": "1"}) != NetworkTopologyStrategy(
            {"dc1": 2}
        )
...
class NameEscapingTest(unittest.TestCase):
    ...
    def test_protect_names(self):
        ...
        assert protect_names(["tests", "test's", "tests ?!@#$%^&*()", "1"]) == [
            "tests",
            '"test\'s"',
            '"tests ?!@#$%^&*()"',
            '"1"',
...
class GetReplicasTest(unittest.TestCase):
    def _get_replicas(self, token_klass):
        ...
        hosts = [
            Host("ip%d" % i, SimpleConvictionPolicy, host_id=uuid.uuid4())
...
class BytesTokensTest(unittest.TestCase):
    def test_bytes_tokens(self):
        ...
        assert bytes_token.hash_fn(123) == 123
        assert bytes_token.hash_fn(str(cassandra.metadata.MAX_LONG)) == str(
            cassandra.metadata.MAX_LONG
        )
...
class KeyspaceMetadataTest(unittest.TestCase):
    def test_export_as_string_user_types(self):
        ...
        keyspace = KeyspaceMetadata(
            keyspace_name, True, "NetworkTopologyStrategy", dict(dc1=3)
        )
...
class FunctionToCQLTests(unittest.TestCase):
    ...
    def test_non_monotonic(self):
        assert (
            "MONOTONIC"
            not in self._function_with_kwargs(
    ...
    def test_monotonic_one(self):
        ...
        assert "MONOTONIC ON x\n    LANG" in mono_on_function.as_cql_query(
            formatted=True
        )
...
class AggregateToCQLTests(unittest.TestCase):
    ...
    def test_deterministic(self):
        for formatted in (True, False):
            ...
            assert query.endswith("DETERMINISTIC"), (
                "'DETERMINISTIC' not found in {}".format(query)
            )
...
class HostsTests(unittest.TestCase):
    def test_iterate_all_hosts_and_modify(self):
        ...
        """
        metadata = Metadata()
        metadata.add_or_return_host(
            Host("dc1.1", SimpleConvictionPolicy, host_id=uuid.uuid4())
        )
        metadata.add_or_return_host(
            Host("dc1.2", SimpleConvictionPolicy, host_id=uuid.uuid4())
        )
