...
class ReplicationFactorTest(unittest.TestCase):
    ...
    def test_replication_factor_equality(self):
        self.assertEqual(
            ReplicationFactor.create("3/1"), ReplicationFactor.create("3/1")
        )
        self.assertEqual(ReplicationFactor.create("3"), ReplicationFactor.create("3"))
        self.assertNotEqual(
            ReplicationFactor.create("3"), ReplicationFactor.create("3/1")
        )
        self.assertNotEqual(
            ReplicationFactor.create("3"), ReplicationFactor.create("3/1")
        )
...
class StrategiesTest(unittest.TestCase):
    ...
    def test_replication_strategy(self):
        ...
        self.assertEqual(
            rs.create("OldNetworkTopologyStrategy", None),
        ...
        self.assertEqual(
            uks, _UnknownStrategy("OldNetworkTopologyStrategy", fake_options_map)
        )
        ...
        self.assertEqual(
            rs.create(
                "NetworkTopologyStrategy", fake_options_map
        ...
        fake_options_map = {"options": "map"}
        self.assertIsInstance(
            rs.create("LocalStrategy", fake_options_map), LocalStrategy
        )
        ...
        self.assertEqual(
            rs.create("xxxxxxxx", fake_options_map),
    ...
    def test_nts_replication_type_parsing(self):
        ...
        hosts = [
            Host("dc1.{}".format(host), SimpleConvictionPolicy) for host in range(3)
        ]
        for host in hosts:
            host.datacenter = "dc1"
    ...
    def test_transient_replication_parsing(self):
        ...
        self.assertEqual(
            nts_transient.dc_replication_factors_info["dc1"], ReplicationFactor(3, 1)
        )
        self.assertEqual(nts_transient.dc_replication_factors["dc1"], 2)
        self.assertIn("'dc1': '3/1'", nts_transient.export_for_schema())
    ...
    def test_nts_replication_parsing(self):
        ...
        self.assertEqual(
            nts_int.dc_replication_factors_info["dc1"], ReplicationFactor(3)
        )
        self.assertEqual(
            nts_str.dc_replication_factors_info["dc1"], ReplicationFactor(3)
        )
    ...
    def test_nts_transient_parsing(self):
        ...
        nts_transient = rs.create(
            "NetworkTopologyStrategy", {"dc1": "3/1", "dc2": "5/1"}
        )
        self.assertEqual(
            nts_transient.dc_replication_factors_info["dc1"], ReplicationFactor(3, 1)
        )
        self.assertEqual(
            nts_transient.dc_replication_factors_info["dc2"], ReplicationFactor(5, 1)
        )
        self.assertEqual(nts_transient.dc_replication_factors["dc1"], 2)
    ...
    def test_nts_make_token_replica_map(self):
        ...
        self.assertItemsEqual(
            replica_map[MD5Token(0)], (dc1_1, dc1_2, dc2_1, dc2_2, dc3_1)
        )
    ...
    def test_nts_make_token_replica_map(self):
        ...
        rf1_replicas = NetworkTopologyStrategy({"dc1": "1"}).make_token_replica_map(
        ...
        rf2_replicas = NetworkTopologyStrategy({"dc1": "2"}).make_token_replica_map(
            token_to_host_owner, ring
        )
        ...
        rf3_replicas = NetworkTopologyStrategy({"dc1": "3"}).make_token_replica_map(
            token_to_host_owner, ring
        )
...
class NameEscapingTest(unittest.TestCase):
    ...
    def test_is_valid_name(self):
        ...
        invalid_keywords = (
            cassandra.metadata.cql_keywords - cassandra.metadata.cql_keywords_unreserved
        )
...
class Murmur3TokensTest(unittest.TestCase):
    ...
    def test_python_vs_c(self):
        ...
        try:
            ...
            for _ in range(iterations):
                for len in range(
                    0, 32
                ):  # zero to one block plus full range of tail lengths
                    key = os.urandom(len)
    ...
    def _verify_hash(self, fn):
        ...
        self.assertEqual(
            fn(str(cassandra.metadata.MAX_LONG).encode()), 7162290910810015547
        )
...
class MD5TokensTest(unittest.TestCase):
    def test_md5_tokens(self):
        ...
        self.assertEqual(
            md5_token.hash_fn("123"), 42767516990368493138776584305024125808
        )
        self.assertEqual(
            md5_token.hash_fn(str(cassandra.metadata.MAX_LONG)),
            28528976619278518853815276204542453639,
        )
...
class BytesTokensTest(unittest.TestCase):
    def test_bytes_tokens(self):
        ...
        self.assertEqual(bytes_token.hash_fn(123), 123)
        self.assertEqual(
            bytes_token.hash_fn(str(cassandra.metadata.MAX_LONG)),
            str(cassandra.metadata.MAX_LONG),
        )
...
class KeyspaceMetadataTest(unittest.TestCase):
    def test_export_as_string_user_types(self):
        ...
        keyspace = KeyspaceMetadata(
            keyspace_name, True, "NetworkTopologyStrategy", {"dc1": 3}
        )
        keyspace.user_types["a"] = UserType(
            keyspace_name, "a", ["one", "two"], ["c", "int"]
        )
        keyspace.user_types["b"] = UserType(
            keyspace_name, "b", ["one", "two", "three"], ["d", "int", "a"]
        )
...
class FunctionToCQLTests(unittest.TestCase):
    ...
    def _function_with_kwargs(self, **kwargs):
        return Function(**dict(self.base_vars, **kwargs))

    def test_non_monotonic(self):
        self.assertNotIn(
            "MONOTONIC",
            self._function_with_kwargs(
                monotonic=False, monotonic_on=()
            ).export_as_string(),
        )

    def test_monotonic_all(self):
        mono_function = self._function_with_kwargs(monotonic=True, monotonic_on=())
        self.assertIn("MONOTONIC LANG", mono_function.as_cql_query(formatted=False))
        self.assertIn("MONOTONIC\n    LANG", mono_function.as_cql_query(formatted=True))
    ...
    def test_monotonic_one(self):
        ...
        self.assertIn(
            "MONOTONIC ON x LANG", mono_on_function.as_cql_query(formatted=False)
        )
        self.assertIn(
            "MONOTONIC ON x\n    LANG", mono_on_function.as_cql_query(formatted=True)
        )

    def test_nondeterministic(self):
        self.assertNotIn(
            "DETERMINISTIC",
            self._function_with_kwargs(deterministic=False).as_cql_query(
                formatted=False
            ),
        )
    ...
    def test_deterministic(self):
        ...
        self.assertIn(
            "DETERMINISTIC\n",
            self._function_with_kwargs(deterministic=True).as_cql_query(formatted=True),
        )
...
class AggregateToCQLTests(unittest.TestCase):
    ...
    def _aggregate_with_kwargs(self, **kwargs):
        return Aggregate(**dict(self.base_vars, **kwargs))

    def test_nondeterministic(self):
        self.assertNotIn(
            "DETERMINISTIC",
            self._aggregate_with_kwargs(deterministic=False).as_cql_query(
                formatted=True
            ),
        )
    ...
    def test_deterministic(self):
        for formatted in (True, False):
            query = self._aggregate_with_kwargs(deterministic=True).as_cql_query(
                formatted=formatted
            )
            self.assertTrue(
                query.endswith("DETERMINISTIC"),
