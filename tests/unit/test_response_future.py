...
class ResponseFutureTests(unittest.TestCase):
    ...
    def test_execute_after_prepare_updates_result_metadata_id(self):
        ...
        rf.prepared_statement = Mock()
        rf.prepared_statement.query_id = query_id
        rf.prepared_statement.result_metadata = [('ks', 'tb', 'old_col', Mock())]
        rf.prepared_statement.result_metadata_id = b'old_hash'
    ...
    def test_execute_after_prepare_no_metadata_id_in_response(self):
        """
        ...
        must leave the existing result_metadata_id on the prepared statement unchanged.
        """
        ...
        # result_metadata is refreshed (always), but result_metadata_id is unchanged.
        assert rf.prepared_statement.result_metadata is new_meta
        assert rf.prepared_statement.result_metadata_id == b'old_hash'
    ...
    def test_query_no_skip_meta_without_extension(self):
        ...
        even when the prepared statement has both a result_metadata_id and valid
        cached result_metadata.  The use_metadata_id=False guard is the deciding
        factor here; the test uses truthy result_metadata to isolate it.
        """
        session = self.make_basic_session()
        session.cluster._default_load_balancing_policy.make_query_plan.return_value = ['ip1']
        session._pools.get.return_value = self.make_pool()
        ...
        ps = Mock()
        ps.result_metadata = [('ks', 'tbl', 'col', Mock())]  # truthy: isolates use_metadata_id guard
        ps.result_metadata_id = b'meta_hash'
