...
class QueryTests(BasicSharedKeyspaceUnitTestCase):
    ...
    def test_incomplete_query_trace(self):
        ...
        self.assertRaises(
            TraceUnavailable, trace.populate, max_wait=0.2, wait_for_complete=True
        )
    ...
    def test_column_names(self):
        ...
        self.assertEqual(
            result_set.column_names, ["user", "game", "year", "month", "day", "score"]
        )
...
class PreparedStatementArgTest(unittest.TestCase):
    ...
    def test_prepare_on_all_hosts(self):
        ...
        self.assertEqual(
            2, self.mock_handler.get_message_count("debug", "Re-preparing")
        )
    ...
    def test_prepare_batch_statement(self):
        ...
        self.assertEqual(
            1, self.mock_handler.get_message_count("debug", "Re-preparing")
        )
    ...
    def test_prepare_batch_statement_after_alter(self):
        ...
        # To verify our test assumption that queries are getting re-prepared properly
        self.assertEqual(
            3, self.mock_handler.get_message_count("debug", "Re-preparing")
        )
...
class SerialConsistencyTests(unittest.TestCase):
    ...
    def test_conditional_update(self):
        ...
        self.assertEqual(
            future.message.serial_consistency_level, ConsistencyLevel.SERIAL
        )
        ...
        result = future.result()
        self.assertEqual(
            future.message.serial_consistency_level, ConsistencyLevel.LOCAL_SERIAL
        )
        self.assertTrue(result)
...
class PreparedWithKeyspaceTests(BaseKeyspaceTests, unittest.TestCase):
    ...
    def test_prepared_with_keyspace_explicit(self):
        ...
        self.assertNotEqual(
            prepared_statement.query_id, prepared_statement_alternative.query_id
        )
