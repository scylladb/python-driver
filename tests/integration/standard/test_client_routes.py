...
class TcpProxy:
    ...
    def _handle_new_connection(self, client_sock):
        try:
...
class NLBEmulator:
    ...
    def start(self, node_addresses):
        ...
        def rr_handler(client_sock):
            ...
            self._rr_index += 1
            addr = addrs[idx]
            self._discovery_proxy.target_host = addr
            self._discovery_proxy.target_port = self.native_port
            original_handler(client_sock)
...
def wait_for_routes_visible(session, connection_id, expected_count, timeout=10, poll_interval=0.1):
    """
    Poll system.client_routes until the expected number of routes for
    *connection_id* are visible via CQL.  Routes posted through the REST
    API propagate asynchronously; this avoids a race between the POST
    returning and the CQL table being readable.

    :param session: an active driver Session (direct, not through NLB)
    :param connection_id: the connection_id string to filter on
    :param expected_count: how many rows we expect to see
    ...
    while True:
        ...
        if time.time() >= deadline:
            raise RuntimeError(
                ...
                "in system.client_routes; found %d" % (expected_count, connection_id, len(rows))
            )
...
class TestGetHostPortMapping(unittest.TestCase):
    ...
    def _query_and_compare(self, connection_ids, host_ids, expected):
        ssl_enabled = self.cluster._client_routes_handler.ssl_enabled
        routes = self.cluster._client_routes_handler._query_routes(
        ...
        # _query_routes picks tls_port or port based on ssl_enabled,
        # so adjust expected accordingly.
        port_key = 'tls_port' if ssl_enabled else 'port'
        adjusted = [
            {
    ...
    def test_get_concrete_host(self):
        filtered = [
            ...
            and r['host_id'] == self.host_ids[0]
        ]
        self._query_and_compare([self.connection_ids[0]], [self.host_ids[0]], filtered)
        ...
        with self._make_client_routes_cluster() as cluster:
            ...
            for node_id in self.node_addrs:
                ...
                self.assertGreater(proxy.total_connections, 0,
                                   f"Node {node_id} proxy saw no connections")
    ...
    def test_should_survive_full_node_replacement_through_nlb(self):
        ...
        with NLBEmulator(
            ...
            with Cluster(
                ...
                handler.handle_control_connection_reconnect(cluster.control_connection)
