...
from cassandra.client_routes import ClientRoutesConfig, ClientRoutesEndpoint, _ClientRoutesHandler
...
class TestGetHostPortMapping(unittest.TestCase):
    ...
    def _query_and_compare(self, connection_ids, host_ids, expected):
        """
        Query routes using _ClientRoutesHandler._query_routes and compare with expected.
        ...
        config = ClientRoutesConfig(
            endpoints=[ClientRoutesEndpoint(
                connection_id=str(self.connection_ids[0]),
                connection_addr="127.0.0.1"
            )])
        config.table_name="gocql_test.client_routes"
