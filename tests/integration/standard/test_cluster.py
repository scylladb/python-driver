...
class ClusterTests(unittest.TestCase):
    ...
    def test_raise_error_on_control_connection_timeout(self):
        ...
        with pytest.raises(
            NoHostAvailable,
            match=r"OperationTimedOut\('errors=Timed out creating connection \(1 seconds\)",
        ):
            cluster.connect()
        cluster.shutdown()
    ...
    def test_connect_on_keyspace(self):
        ...
        assert [(8889, 8889)] == result, "Rows in ResultSet are {0}".format(
            result.current_rows
        )
    ...
    def test_refresh_schema_no_wait(self):
        ...
        def patched_wait_for_responses(*args, **kwargs):
            ...
            if (
                ...
                and "SELECT schema_version FROM system.local WHERE key='local'"
                in args[2].query
            ):
                new_uuid = uuid4()
        ...
        while time.time() < deadline:
            ...
            for holder in cluster.get_connection_holders():
                if not isinstance(holder, HostConnection):
                    continue
                if (
                    holder.host.sharding_info
                    and len(holder._connections)
                    < holder.host.sharding_info.shards_count
                ):
                    all_connected = False
                    break
            if all_connected:
            ...
            time.sleep(0.1)
        raise RuntimeError(
            "Timed out waiting for all shard connections to be established"
        )
    ...
    def test_idle_heartbeat(self):
        ...
        ] * max(num_connections * 2, len(cluster.metadata.all_hosts()))
        ...
        non_idle = [
            c for c in connections if not c.is_control_connection and not c.is_idle
        ]
    ...
    def test_profile_load_balancing(self):
        ...
        with TestCluster(
            ...
            assert (
                session.execute(query, execution_profile="node1").one().release_version
            )
    ...
    def test_clone_shared_lbp(self):
        ...
        with TestCluster(execution_profiles=exec_profiles) as cluster:
            session = cluster.connect(wait_for_all_pools=True)
            assert len(cluster.metadata.all_hosts()) > 1, (
    ...
    def test_profile_pool_management(self):
        ...
        with TestCluster(
            ...
            pools = session.get_pool_state()
            assert set(h.address for h in pools) == set(
    ...
    def test_stale_connections_after_shutdown(self):
        ...
        for _ in range(10):
            with TestCluster(protocol_version=3) as cluster:
                cluster.connect(wait_for_all_pools=True).execute(
            ...
            result = subprocess.run(
                ["lsof -nP | awk '$3 ~ \":9042\" {print $0}' | grep ''"],
                shell=True,
                capture_output=True,
            )
    ...
    def test_compact_option(self):
        ...
        assert set(nc_results.current_rows) == {
            (1, "a1", 11, 11, "b1"),
            (1, "a11", 11, 11, "b11"),
            (2, "a2", 22, 22, "b2"),
            (2, "a22", 22, 22, "b22"),
        ...
        assert set(results.current_rows) == {
            (1, 11, 11),
            (2, 22, 22),
            (3, 33, 33),
            (4, 44, 44),
    ...
    def _assert_replica_queried(self, trace, only_replicas=True):
        ...
        if only_replicas:
            assert len(queried_hosts) == 1, "The hosts queried where {}".format(
                queried_hosts
            )
        else:
            assert len(queried_hosts) > 1, "The host queried was {}".format(
                queried_hosts
            )
...
class DeprecationWarningTest(unittest.TestCase):
    def test_deprecation_warnings_legacy_parameters(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert any(
                [
                    "Legacy execution parameters will be removed in 4.0. "
                    "Consider using execution profiles." in str(wa.message)
                    for wa in w
                ]
            )
