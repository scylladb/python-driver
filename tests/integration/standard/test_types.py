    ...
    def test_large_timestamp_precision(self):
        ...
        for original_timestamp in test_timestamps:
            with self.subTest(timestamp=original_timestamp):
                ...
                # Retrieve the timestamp (uses deserialization) - use a prepared
                # statement since "?" placeholders are only valid for bound statements
                select = s.prepare("SELECT pk FROM large_timestamps WHERE pk = ?")
                result = s.execute(select, [original_timestamp]).one()
                ...
                result2 = s.execute(select, [retrieved_timestamp]).one()
                ...
                delete = s.prepare("DELETE FROM large_timestamps WHERE pk = ?")
                s.execute(delete, [original_timestamp])
