...
class TypeTests(BasicSharedKeyspaceUnitTestCase):
    ...
    @unittest.skipIf(
        not hasattr(cassandra, "deserializers"),
        "Cython required for to test DesBytesTypeArray deserializer",
    )
    ...
    def test_can_insert_primitive_datatypes(self):
        ...
        # verify data
        results = s.execute(
            "SELECT {0} FROM alltypes WHERE zz=0".format(columns_string)
        ).one()
        for expected, actual in zip(params, results):
        ...
        for i, datatype in enumerate(PRIMITIVE_DATATYPES):
            ...
            for j, data_sample in enumerate(get_all_samples(datatype)):
                ...
                s.execute(
                    "INSERT INTO alltypes ({0}) VALUES ({1})".format(
                        single_columns_string, placeholders
                    ),
                    single_params,
                )
                # verify data
                result = s.execute(
                    "SELECT {0} FROM alltypes WHERE zz=%s".format(
                        single_columns_string
                    ),
                    (key,),
                ).one()[1]
                compare_value = data_sample

                if isinstance(data_sample, ipaddress.IPv4Address) or isinstance(
                    data_sample, ipaddress.IPv6Address
                ):
        ...
        results = s.execute(
            select,
            execution_profile=s.execution_profile_clone_update(
                EXEC_PROFILE_DEFAULT, row_factory=ordered_dict_factory
            ),
        ).one()
    ...
    def test_can_insert_collection_datatypes(self):
        ...
        for i, collection_type in enumerate(COLLECTION_TYPES):
            for j, datatype in enumerate(PRIMITIVE_DATATYPES_KEYS):
                ...
                col_names.append(
                    "{0}_{1}".format(chr(start_index + i), chr(start_index + j))
                )
        ...
        placeholders = ", ".join(["%s"] * len(col_names))
        s.execute(
            "INSERT INTO allcoltypes ({0}) VALUES ({1})".format(
                columns_string, placeholders
            ),
            params,
        )
        ...
        # verify data
        results = s.execute(
            "SELECT {0} FROM allcoltypes WHERE zz=0".format(columns_string)
        ).one()
        for expected, actual in zip(params, results):
            assert actual == expected
        ...
        # verify data with prepared statement query
        select = s.prepare(
            "SELECT {0} FROM allcoltypes WHERE zz=?".format(columns_string)
        )
        results = s.execute(select.bind([0])).one()
        for expected, actual in zip(params, results):
            assert actual == expected
    ...
    def test_can_insert_empty_strings_and_nulls(self):
        ...
        non_string_types = (
            PRIMITIVE_DATATYPES
            - string_types
            - set(("blob", "date", "inet", "time", "timestamp"))
        )
        ...
        execute_until_pass(
            s, "CREATE TABLE all_empty ({0})".format(", ".join(alpha_type_list))
        )
        ...
        results = s.execute(
            "SELECT {0} FROM all_empty WHERE zz=2".format(columns_string)
        ).one()
        assert all(x is None for x in results)

        # verify all types initially null with prepared statement
        select = s.prepare(
            "SELECT {0} FROM all_empty WHERE zz=?".format(columns_string)
        )
        ...
        s.execute(
            "INSERT INTO all_empty (zz, {0}) VALUES (3, {1})".format(
                columns_string, placeholders
            ),
        ...
        results = s.execute(
            "SELECT {0} FROM all_empty WHERE zz=3".format(columns_string)
        ).one()
        ...
        results = s.execute(
            s.prepare("SELECT {0} FROM all_empty WHERE zz=?".format(columns_string)),
            [3],
        ).one()
        for expected, actual in zip(expected_values.values(), results):
            assert actual == expected
        ...
        for col in non_string_columns:
            ...
            insert = s.prepare(
                "INSERT INTO all_empty (zz, {0}) VALUES (4, ?)".format(col)
            )
        ...
        columns_string = ",".join(col_names)
        placeholders = ",".join(["%s"] * len(col_names))
        simple_insert = "INSERT INTO all_empty (zz, {0}) VALUES (5, {1})".format(
            columns_string, placeholders
        )
        ...
        placeholders = ",".join(["?"] * len(col_names))
        insert = s.prepare(
            "INSERT INTO all_empty (zz, {0}) VALUES (5, {1})".format(
                columns_string, placeholders
            )
        )
    ...
    def test_can_insert_empty_values_for_int32(self):
        ...
        execute_until_pass(
            s, "INSERT INTO empty_values (a, b) VALUES ('a', blobAsInt(0x))"
        )
        try:
            Int32Type.support_empty_values = True
            results = execute_until_pass(
                s, "SELECT b FROM empty_values WHERE a='a'"
            ).one()
    ...
    def test_can_insert_tuples(self):
        ...
        s.execute(
            "INSERT INTO tuple_type (a, b) VALUES (0, %s)", parameters=(complete,)
        )
        ...
        s.execute(
            "INSERT INTO tuple_type (a, b) VALUES (2, %s)", parameters=(subpartial,)
        )
    ...
    def test_can_insert_tuples_with_varying_lengths(self):
        ...
        for i in lengths:
            ...
            with pytest.raises(InvalidRequest):
                s.execute(
                    "INSERT INTO tuple_lengths (k, v_%s) VALUES (0, %s)",
                    (i, created_tuple),
                )
            ...
            s.execute(
                "INSERT INTO tuple_lengths (k, v_%s) VALUES (0, %s)", (i, created_tuple)
            )
    ...
    def test_can_insert_tuples_all_primitive_datatypes(self):
        ...
        for i, data_type in enumerate(PRIMITIVE_DATATYPES):
            ...
            s.execute(
                "INSERT INTO tuple_primitive (k, v) VALUES (%s, %s)", (i, tuple(values))
            )
    ...
    def test_can_insert_tuples_all_collection_datatypes(self):
        ...
        for datatype in PRIMITIVE_DATATYPES_KEYS:
            values.append(
                "v_{0} frozen<tuple<list<{1}>>>".format(len(values), datatype)
            )
        ...
        for datatype in PRIMITIVE_DATATYPES_KEYS:
            ...
            if datatype == "blob":
                ...
                datatype_1 = "ascii"
            values.append(
                "v_{0} frozen<tuple<map<{1}, {2}>>>".format(
        ...
        # create table
        s.execute(
            "CREATE TABLE tuple_non_primative ("
            "k int PRIMARY KEY, "
            "%s)" % ", ".join(values)
        )
        ...
        for datatype in PRIMITIVE_DATATYPES_KEYS:
            ...
            result = s.execute(
                "SELECT v_%s FROM tuple_non_primative WHERE k=0", (i,)
            ).one()
    ...
    def test_can_insert_nested_tuples(self):
        ...
        for i in (1, 2, 3, 32):
            ...
            # write tuple
            s.execute(
                "INSERT INTO nested_tuples (k, v_%s) VALUES (%s, %s)",
                (i, i, created_tuple),
            )

            # verify tuple was written and read correctly
            result = s.execute(
                "SELECT v_%s FROM nested_tuples WHERE k=%s", (i, i)
            ).one()
            assert created_tuple == result["v_%s" % i]
        c.shutdown()
    ...
    def test_insert_collection_with_null_fails(self):
        ...
        for collection_type in ["list", "set"]:
            for simple_type in PRIMITIVE_DATATYPES_KEYS:
                columns.append(
                    f"{collection_type}_{simple_type} {collection_type}<{simple_type}>"
                )
        for simple_type in PRIMITIVE_DATATYPES_KEYS:
            columns.append(f"map_k_{simple_type} map<{simple_type}, ascii>")
            columns.append(f"map_v_{simple_type} map<ascii, {simple_type}>")
        s.execute(
            f"CREATE TABLE collection_nulls (k int PRIMARY KEY, {', '.join(columns)})"
        )
        ...
        for simple_type in PRIMITIVE_DATATYPES_KEYS:
            query_str = (
                f"INSERT INTO collection_nulls (k, set_{simple_type}) VALUES (%s, %s)"
            )
        ...
        for simple_type in PRIMITIVE_DATATYPES_KEYS:
            query_str = (
                f"INSERT INTO collection_nulls (k, list_{simple_type}) VALUES (%s, %s)"
            )
        ...
        for simple_type in PRIMITIVE_DATATYPES_KEYS:
            query_str = (
                f"INSERT INTO collection_nulls (k, map_k_{simple_type}) VALUES (%s, %s)"
            )
        ...
        for simple_type in PRIMITIVE_DATATYPES_KEYS:
            query_str = (
                f"INSERT INTO collection_nulls (k, map_v_{simple_type}) VALUES (%s, %s)"
            )
    ...
    def test_special_float_cql_encoding(self):
        ...
        # cql encoding
        verify_insert_select(
            "INSERT INTO float_cql_encoding (f, d) VALUES (%s, %s)",
        ...
        # prepared binding
        verify_insert_select(
            s.prepare("INSERT INTO float_cql_encoding (f, d) VALUES (?, ?)"),
    ...
    def test_cython_decimal(self):
        ...
        self.session.execute(
            "CREATE TABLE {0} (dc decimal PRIMARY KEY)".format(self.function_table_name)
        )
        try:
            self.session.execute(
                "INSERT INTO {0} (dc) VALUES (-1.08430792318105707)".format(
                    self.function_table_name
                )
            )
            results = self.session.execute(
                "SELECT * FROM {0}".format(self.function_table_name)
            )
    ...
    def test_smoke_duration_values(self):
        ...
        for nanosecond_value in nanosecond_smoke_values:
            for month_day_value in month_day_smoke_values:
                ...
                self.session.execute(
                    prepared,
                    (1, Duration(month_day_value, month_day_value, nanosecond_value)),
                )
                results = self.session.execute("SELECT * FROM duration_smoke")

                v = results.one()[1]
                assert (
                    Duration(month_day_value, month_day_value, nanosecond_value) == v
        ...
        with pytest.raises(ValueError):
            self.session.execute(
                prepared, (1, Duration(0, 0, int("8FFFFFFFFFFFFFF0", 16)))
            )
        with pytest.raises(ValueError):
            self.session.execute(
                prepared, (1, Duration(0, int("8FFFFFFFFFFFFFF0", 16), 0))
            )
        with pytest.raises(ValueError):
            self.session.execute(
                prepared, (1, Duration(int("8FFFFFFFFFFFFFF0", 16), 0, 0))
            )
...
class TypeTestsProtocol(BasicSharedKeyspaceUnitTestCase):
    ...
    def test_nested_types_with_protocol_version(self):
        ...
        self.session.execute(
            "CREATE TYPE {0}.typ (v0 frozen<map<int, frozen<list<int>>>>, v1 frozen<list<int>>)".format(
                self.keyspace_name
            )
        )
    ...
    def read_inserts_at_level(self, proto_ver):
        ...
        try:
            ...
            assert (
                "{SortedSet([1, 2]): [1, 2, 3], SortedSet([3, 5]): [4, 5, 6]}"
                == str(results.v)
            )

            results = session.execute("select * from w").one()
            assert (
                "typ(v0=OrderedMapSerializedKey([(1, [1, 2, 3]), (2, [4, 5, 6])]), v1=[7, 8, 9])"
                == str(results.v)
            )
    ...
    def run_inserts_at_version(self, proto_ver):
        ...
        try:
            ...
            session.execute(
                p, (0, {(1, 2): [1, 2, 3], (3, 5): [4, 5, 6]}, (123, "four"))
            )
...
class TypeTestsVector(BasicSharedKeyspaceUnitTestCase):
    ...
    def _get_row_simple(self, idx, table_name):
        rs = self.session.execute(
            "select j from {0}.{1} where i = {2}".format(
                self.keyspace_name, table_name, idx
            )
        )
    ...
    def _round_trip_test(
        self, subtype, subtype_fn, test_fn, use_positional_parameters=True
    ):

        table_name = (
            subtype.replace("<", "A").replace(">", "B").replace(",", "C") + "isH"
        )
    ...
    def test_round_trip_integers(self):
        ...
        self._round_trip_test(
            "smallint", partial(random.randint, 0, 2**15), assertEqual
        )
        self._round_trip_test(
            "tinyint", partial(random.randint, 0, (2**7) - 1), assertEqual
        )
    ...
    def test_round_trip_floating_point(self):
        ...
        self._round_trip_test(
            "float", partial(random.uniform, 0.0, 100.0), _almost_equal_test_fn
        )
        self._round_trip_test(
            "double", partial(random.uniform, 0.0, 100.0), _almost_equal_test_fn
        )
    ...
    def test_round_trip_date_and_time(self):
        ...
        def _random_datetime():
            return datetime.today() - timedelta(
                hours=random.randint(0, 18), days=random.randint(1, 1000)
            )
    ...
    def test_round_trip_miscellany(self):
        ...
        def _random_duration():
            return Duration(
                random.randint(0, 11), random.randint(0, 11), random.randint(0, 10000)
            )
    ...
    def test_round_trip_tuples(self):
        ...
        self._round_trip_test(
            ...
            _random_tuple,
            assertEqual,
            use_positional_parameters=False,
        )
        self._round_trip_test(
            "tuple<int,varint>",
            _random_tuple,
    ...
    def test_round_trip_udts(self):
        ...
        self.session.execute(
            "create type {}.fixed_type (a int, b int)".format(self.keyspace_name)
        )
        self.session.execute(
            "create type {}.mixed_type_one (a int, b varint)".format(self.keyspace_name)
        )
        self.session.execute(
            "create type {}.mixed_type_two (a varint, b int)".format(self.keyspace_name)
        )
        self.session.execute(
            "create type {}.var_type (a varint, b varint)".format(self.keyspace_name)
        )
        ...
        self.cluster.register_user_type(
            self.keyspace_name, "mixed_type_one", GeneralUDT
        )
        self.cluster.register_user_type(
            self.keyspace_name, "mixed_type_two", GeneralUDT
        )
