...
class BasicDseAuthTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        if isinstance(ccm_cluster, DseCluster):
            ...
            ccm_cluster.start(
                wait_for_binary_proto=True, wait_other_notice=True, jvm_args=[krb5java]
            )
        else:
    ...
    def refresh_kerberos_tickets(self, keytab_file, user_name, krb_conf):
        ...
        """
        self.ads_pid = subprocess.call(
            ["kinit", "-t", keytab_file, user_name],
    ...
    def test_should_not_authenticate_with_bad_user_ticket(self):
        ...
        """
        self.refresh_kerberos_tickets(
            self.dseuser_keytab, "dseuser@DATASTAX.COM", self.krb_conf
        )
    ...
    def test_connect_with_kerberos(self):
        ...
        """
        self.refresh_kerberos_tickets(
            self.cassandra_keytab, "cassandra@DATASTAX.COM", self.krb_conf
        )
        ...
        connections = [
            c
            for holders in self.cluster.get_connection_holders()
        ...
        for connection in connections:
            self.assertTrue(
                "DseAuthenticator"
                in connection.authenticator.server_authenticator_class
            )
    ...
    def test_connect_with_kerberos_and_graph(self):
        ...
        ].graph_options.graph_name = self._testMethodName.lower()
    ...
    def test_connect_with_explicit_principal(self):
        ...
        auth_provider = DSEGSSAPIAuthProvider(
            service="dse", qops=["auth"], principal="cassandra@DATASTAX.COM"
        )
        ...
        auth_provider = DSEGSSAPIAuthProvider(
            service="dse", qops=["auth"], principal="notauser@DATASTAX.COM"
        )
        self.assertRaises(NoHostAvailable, self.connect_and_query, auth_provider)
    ...
    def test_proxy_login_with_kerberos(self):
        ...
        self.refresh_kerberos_tickets(
            self.charlie_keytab, "charlie@DATASTAX.COM", self.krb_conf
        )
        auth_provider = DSEGSSAPIAuthProvider(
            service="dse", qops=["auth"], principal="charlie@DATASTAX.COM"
        )
        self.connect_and_query(auth_provider, query=query)

        # Try proxy login with bob
        self.refresh_kerberos_tickets(
            self.bob_keytab, "bob@DATASTAX.COM", self.krb_conf
        )
        auth_provider = DSEGSSAPIAuthProvider(
            service="dse",
            qops=["auth"],
            principal="bob@DATASTAX.COM",
        ...
        auth_provider = DSEGSSAPIAuthProvider(
            service="dse", qops=["auth"], principal="bob@DATASTAX.COM"
        )
        self.assertRaises(
            Unauthorized, self.connect_and_query, auth_provider, query=query
        )
    ...
    def test_proxy_login_with_kerberos_forbidden(self):
        ...
        self.assertRaises(
            NoHostAvailable, self.connect_and_query, auth_provider, query=query
        )
    ...
    def _remove_proxy_setup(self):
        ...
        session.execute(
            "REVOKE PROXY.LOGIN ON ROLE '{0}' FROM '{1}'".format(
                "charlie@DATASTAX.COM", "bob@DATASTAX.COM"
            )
        )
    ...
    def _setup_for_proxy(self, grant=True):
        ...
        stmts = [
            ...
            "CREATE ROLE IF NOT EXISTS '{0}' WITH LOGIN = TRUE;".format(
                "charlie@DATASTAX.COM"
            ),
            "GRANT EXECUTE ON ALL AUTHENTICATION SCHEMES to 'charlie@DATASTAX.COM'",
            # Create a keyspace and allow only charlie to query it.
            "CREATE KEYSPACE testkrbproxy WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}",
        ...
        if grant:
            stmts.append(
                "GRANT PROXY.LOGIN ON ROLE '{0}' to '{1}'".format(
                    "charlie@DATASTAX.COM", "bob@DATASTAX.COM"
                )
            )
...
class BaseDseProxyAuthTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        stmts = [
            ...
            "CREATE KEYSPACE testproxy WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}",
    ...
    def tearDown(self):
        ...
        self.root_session.execute(
            "REVOKE PROXY.LOGIN ON ROLE {0} from {1}".format(
                self.user_role, self.server_role
            )
        )
        self.root_session.execute(
            "REVOKE PROXY.EXECUTE ON ROLE {0} from {1}".format(
                self.user_role, self.server_role
            )
        )
    ...
    def grant_proxy_execute(self):
        ...
        """
        self.root_session.execute(
            "GRANT PROXY.EXECUTE on role {0} to {1}".format(
                self.user_role, self.server_role
            )
        )
...
class DseProxyAuthTest(BaseDseProxyAuthTest):
    ...
    def connect_and_query(
        self, auth_provider, execute_as=None, query="SELECT * FROM testproxy.testproxy"
    ):
        self.cluster = TestCluster(auth_provider=auth_provider)
        self.session = self.cluster.connect()
    ...
    def test_connection_with_transitional_mode(self):
        ...
        self.assertIsNotNone(
            self.connect_and_query(auth_provider, query="SELECT * from system.local")
        )
