...
class FunctionAndAggregateMetadataTests(BasicSharedKeyspaceUnitTestCaseRF1):
    ...
    def _populated_ks_meta_attr(self, attr_name):
        ...
        while not val:
            ...
            val = getattr(
                self.cluster.metadata.keyspaces[self.keyspace_name], attr_name
            )
            self.assertLess(
                time.time(), start_time + 30, "did not see func in metadata in 30s"
            )
    ...
    def test_monotonic_on_and_deterministic_function(self):
        ...
        """.format(ksn=self.keyspace_name, ftn=self.func_name)
        )
        ...
        self.session.execute(
            "DROP FUNCTION {}.{}".format(self.keyspace_name, self.func_name)
        )
    ...
    def test_deterministic_aggregate(self):
        ...
        self.session.execute(
            ...
            INITCOND 0
            DETERMINISTIC
        """.format(ksn=self.keyspace_name, ftn=self.func_name, an=self.agg_name)
        )
        ...
        self.session.execute(
            "DROP AGGREGATE {}.{}".format(self.keyspace_name, self.agg_name)
        )
    ...
    def test_nondeterministic_aggregate(self):
        ...
        self.assertEqual(
            "CREATE AGGREGATE {ksn}.{an} SFUNC {ftn} STYPE int INITCOND 0".format(
                ksn=self.keyspace_name, ftn=self.func_name, an=self.agg_name
            ),
            ag.as_cql_query(),
        )
        self.session.execute(
...
class RLACMetadataTests(BasicSharedKeyspaceUnitTestCase):
    ...
    def test_rlac_on_table(self):
        ...
        restrict_cql = "RESTRICT ROWS ON {0}.reports USING report_user".format(
            self.keyspace_name
        )
        self.session.execute(restrict_cql)
        table_meta = self.cluster.metadata.keyspaces[self.keyspace_name].tables[
            "reports"
        ]
    ...
    def test_rlac_on_mv(self):
        ...
        self.session.execute(
            "CREATE MATERIALIZED VIEW {0}.reports_by_year AS "
            ...
            " PRIMARY KEY ((report_year, report_user), report_number)".format(
                self.keyspace_name
            )
        )

        restrict_cql_table = "RESTRICT ROWS ON {0}.reports2 USING report_user".format(
            self.keyspace_name
        )
        self.session.execute(restrict_cql_table)
        restrict_cql_view = (
            "RESTRICT ROWS ON {0}.reports_by_year USING report_user".format(
                self.keyspace_name
            )
        )
...
class GraphMetadataTests(BasicExistingKeyspaceUnitTestCase):
    ...
    def setUpClass(cls):
        if DSE_VERSION and DSE_VERSION >= Version("6.8"):
            ...
            cls.session.execute("""
            CREATE KEYSPACE ks_no_graph_engine WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1};
            """)
            cls.session.execute(
                """
            CREATE KEYSPACE %s WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} and graph_engine = 'Core';
            """
                % (cls.ks_name,)
            )
    ...
    def test_keyspace_metadata(self):
        self.assertIsNone(
            self.cluster.metadata.keyspaces["ks_no_graph_engine"].graph_engine, None
        )
        self.assertEqual(
            self.cluster.metadata.keyspaces[self.ks_name].graph_engine, "Core"
        )

    def test_keyspace_metadata_alter_graph_engine(self):
        self.session.execute(
            "ALTER KEYSPACE %s WITH graph_engine = 'Tinker'" % (self.ks_name,)
        )
        self.assertEqual(
            self.cluster.metadata.keyspaces[self.ks_name].graph_engine, "Tinker"
        )
        self.session.execute(
            "ALTER KEYSPACE %s WITH graph_engine = 'Core'" % (self.ks_name,)
        )
        self.assertEqual(
            self.cluster.metadata.keyspaces[self.ks_name].graph_engine, "Core"
        )
    ...
    def test_vertex_metadata(self):
        vertex_meta = (
            self.cluster.metadata.keyspaces[self.ks_name].tables["person"].vertex
        )
        ...
        vertex_meta = (
            self.cluster.metadata.keyspaces[self.ks_name].tables["software"].vertex
        )
        self.assertEqual(vertex_meta.keyspace_name, self.ks_name)
    ...
    def test_edge_metadata(self):
        edge_meta = (
            self.cluster.metadata.keyspaces[self.ks_name].tables["contributors"].edge
        )
        ...
        self.assertEqual(
            edge_meta.to_partition_key_columns, ["company_name", "software_name"]
        )
