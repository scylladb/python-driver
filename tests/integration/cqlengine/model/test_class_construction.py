...
class TestModelClassFunction(BaseCassEngTestCase):
    ...
    def test_column_ordering_is_preserved(self):
        ...
        assert [x for x in Stuff._columns.keys()] == [
            "id",
            "words",
            "content",
            "numbers",
...
class TestManualTableNaming(BaseCassEngTestCase):
    ...
    def test_proper_table_naming(self):
        assert (
            self.RenamedTest.column_family_name(include_keyspace=False) == "manual_name"
        )
        assert (
            self.RenamedTest.column_family_name(include_keyspace=True)
...
class TestManualTableNamingCaseSensitive(BaseCassEngTestCase):
    ...
    def test_proper_table_naming_case_insensitive(self):
        ...
        """
        assert (
            self.RenamedCaseInsensitiveTest.column_family_name(include_keyspace=False)
            == "manual_name"
        )
        assert (
            self.RenamedCaseInsensitiveTest.column_family_name(include_keyspace=True)
            == "whatever.manual_name"
        )
    ...
    def test_proper_table_naming_case_sensitive(self):
        ...
        assert (
            self.RenamedCaseSensitiveTest.column_family_name(include_keyspace=False)
            == '"Manual_Name"'
        )
        assert (
            self.RenamedCaseSensitiveTest.column_family_name(include_keyspace=True)
            == 'whatever."Manual_Name"'
        )
