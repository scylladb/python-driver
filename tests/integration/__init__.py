...
def _get_cass_version_from_dse(dse_version):
    if dse_version.startswith("4.6") or dse_version.startswith("4.5"):
        ...
        log.error(
            "Unknown dse version found {0}, defaulting to 2.1".format(dse_version)
        )
        cass_ver = "2.1"
...
def _get_dse_version_from_cass(cass_version):
    if cass_version.startswith("2.1"):
        ...
        log.error(
            "Unknown cassandra version found {0}, defaulting to 2.1".format(
                cass_version
            )
        )
        dse_ver = "2.1"
...
protocolv6 = unittest.skipUnless(
    6 in get_supported_protocol_versions(),
    "Protocol versions less than 6 are not supported",
)
...
greaterthanorequaldse68 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION >= Version("6.8"),
    "DSE 6.8 or greater required for this test",
)
greaterthanorequaldse67 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION >= Version("6.7"),
    "DSE 6.7 or greater required for this test",
)
greaterthanorequaldse60 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION >= Version("6.0"),
    "DSE 6.0 or greater required for this test",
)
greaterthanorequaldse51 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION >= Version("5.1"),
    "DSE 5.1 or greater required for this test",
)
greaterthanorequaldse50 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION >= Version("5.0"),
    "DSE 5.0 or greater required for this test",
)
lessthandse51 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION < Version("5.1"), "DSE version less than 5.1 required"
)
lessthandse60 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION < Version("6.0"), "DSE version less than 6.0 required"
)
lessthandse69 = unittest.skipUnless(
    DSE_VERSION and DSE_VERSION < Version("6.9"), "DSE version less than 6.9 required"
)
...
requirescloudproxy = unittest.skipIf(
    CLOUD_PROXY_PATH is None, "Cloud Proxy path hasn't been specified"
)
    ...
    if is_current_cluster(cluster_name, nodes, workloads):
        ...
        try:
            ...
            if dse_cluster:
                ...
                common.switch_cluster(path, cluster_name)
                CCM_CLUSTER.set_configuration_options(configuration_options)
                CCM_CLUSTER.populate(
                    nodes, ipformat=ipformat, use_single_interface=use_single_interface
                )
            else:
                ccm_cluster_clz = (
                    CCMCluster
...
def drop_keyspace_shutdown_cluster(keyspace_name, session, cluster):
    try:
        ...
        log.warning(
            "Error encountered when dropping keyspace {0}".format(keyspace_name)
        )
        ex_type, ex, tb = sys.exc_info()
        log.warning(
            "{0}: {1} Backtrace: {2}".format(
...
def assert_startswith(s, prefix):
    if not s.startswith(prefix):
        raise AssertionError("{} does not start with {}".format(repr(s), repr(prefix)))
