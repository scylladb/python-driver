...
PROTOCOL_VERSION = (
    4
    if PROTOCOL_VERSION in (ProtocolVersion.DSE_V1, ProtocolVersion.DSE_V2)
...
class EmptyColumnTests(SimulacronCluster):
    ...
    def test_empty_columns_with_all_row_factories(self):
        ...
        self.assertEqual(list(self.session.execute(query)), [("testval", "testval1")])

        self.cluster.profile_manager.profiles[
            EXEC_PROFILE_DEFAULT
        ].row_factory = dict_factory
        self.assertEqual(
            list(self.session.execute(query)), [{"": "testval", " ": "testval1"}]
        )
