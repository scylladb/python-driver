...
class CloudTests(CloudProxyCluster):
    ...
    def test_support_overriding_auth_provider(self):
        try:
            self.connect(
                self.creds, auth_provider=PlainTextAuthProvider("invalid", "invalid")
            )
        except:
    ...
    def test_error_overriding_ssl_context(self):
        ...
        self.assertIn(
            "cannot be specified with a cloud configuration", str(cm.exception)
        )
    ...
    def test_verify_hostname(self):
        with patch(
            "cassandra.datastax.cloud.parse_metadata_info",
    ...
    def test_load_balancing_policy_is_dcawaretokenlbp(self):
        ...
        self.assertIsInstance(
            self.cluster.profile_manager.default.load_balancing_policy, TokenAwarePolicy
        )
        self.assertIsInstance(
            self.cluster.profile_manager.default.load_balancing_policy._child_policy,
            DCAwareRoundRobinPolicy,
        )
    ...
    def test_resolve_and_reconnect_on_node_down(self):
        ...
        with patch.object(
            SniEndPoint, "resolve", wraps=host.endpoint.resolve
        ) as mocked_resolve:
    ...
    def test_default_consistency_of_execution_profiles(self):
        ...
        self.cluster.add_execution_profile(
            "post_connect_default_ep", ExecutionProfile()
        )
        self.cluster.add_execution_profile(
        ...
        for changed in [
            ...
            self.assertEqual(
                cl,
                ConsistencyLevel.LOCAL_ONE,
                "Expecting LOCAL ONE for profile {}, but got {} instead".format(
                    default, cl
                ),
            )
