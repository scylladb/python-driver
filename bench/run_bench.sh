...
for v in "${VARIANT_LIST[@]}"; do
    ...
    rc=0
    result=$(taskset -c "$CLIENT_CPUSET" env $env_prefix $python_bin -I "$BENCH_SCRIPT" \
        --variant "$v" \
        --host "$HOST" \
        --port "$PORT" \
    ...
    if [ "$rc" -ne 0 ]; then
        echo "  ERROR: Variant $v failed (exit code $rc). Skipping."
        echo ""
        continue
    fi
...
print(f\"  => {r['rows']:,} rows over {r['runs']} run(s): \"
      f\"best={r['best']:,.0f}  avg={r['avg']:,.0f}  worst={r['worst']:,.0f} rows/sec\")
"
    RESULT_JSONS+=("$result")
    echo ""
done
