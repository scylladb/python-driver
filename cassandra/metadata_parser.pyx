...
def make_recv_results_metadata(dict type_codes_map, object CUSTOM_TYPE,
    ...
    def recv_results_metadata(self, BytesIOReader reader, user_type_map):
        ...
        cdef object coltype
        cdef int i
        # colcount is peer-controlled; bound it against what the remaining
        # buffer could actually contain (each column needs at least a
        # 2-byte name length + 2-byte type code) before allocating, so a
        # truncated/malformed frame claiming e.g. 0x7fffffff columns fails
        # fast instead of triggering a multi-GB allocation.
        if colcount < 0 or colcount > (reader.size - reader.pos) // 4:
            raise ValueError(
                "Invalid column count %d for remaining buffer of %d bytes"
                % (colcount, reader.size - reader.pos))
