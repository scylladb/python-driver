...
class LWTRetryPolicy(ExponentialBackoffRetryPolicy):
    ...
    LWT queries use Paxos consensus, driven by the coordinator node the driver
    contacts for the request (not necessarily the first replica in the token
    ring). Retrying LWT queries on a *different* host causes Paxos contention —
    the new coordinator must compete with the original one, potentially causing
    cascading timeouts.

    This policy addresses that by:

    - **CAS write timeouts**: Retrying on the **same host** (the original coordinator)
