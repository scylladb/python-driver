...
# The `cdef bint` above and this `from ... import` are not a name collision:
# it is the standard idiom (also used in cython_marshal.pyx, cython_utils.pyx,
# and numpy_parser.pyx) for caching a Python-computed bool into a fast C-level
# global -- Cython binds the imported Python object to the already-declared
# C name and coerces it to bint at module init time.
