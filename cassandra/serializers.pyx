...
def obj_array(list objs):
    ...
    Mirrors ``deserializers.obj_array()``'s typed-memoryview convention,
    with one difference: empty input returns the plain (empty) list here,
    since ``cython_array`` does not support zero-length shapes. This is
    safe for this module's own callers (``make_serializers()``, tested
    directly with empty input), but it is defensive only -- it does not,
    by itself, make an empty-input call safe end-to-end for a caller that
    assigns the result to a typed ``Serializer[::1]`` memoryview, since a
    plain list isn't a valid memoryview value. Such a caller must still
    guard against empty input before assignment.
    """
    if not objs:
        return objs
    cdef object[:] arr
