    ...
    def on_add(self, host, refresh_nodes=True,
        ...
        def future_completed(future):
            ...
            if reconcile_pools_on_failure and not self.is_shutdown:
                ...
                # Send the normal DOWN notifications, but remove pools
                # directly: Session.on_down() would start independent
                # reconciliation before this aggregate failure can exclude
                # the replacement and establish retry ownership.
                self.profile_manager.on_down(host)
                self.control_connection.on_down(host)
