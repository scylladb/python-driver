    ...
    * EXPERIMENTAL: :class:`cassandra.io.asyncioreactor.AsyncioConnection`
    ...
    def __init__(self,
        ...
        if client_routes_config is not None and address_translator is not None:
            raise ValueError("client_routes_config and address_translator are mutually exclusive")

        # Handle client routes configuration
        self._client_routes_handler = None
        if client_routes_config is not None:
            from cassandra.client_routes import ClientRoutesConfig, ClientRoutesHandler, ClientRoutesAddressTranslator
            ...
            self._client_routes_handler = ClientRoutesHandler(client_routes_config, ssl_enabled=ssl_enabled)
            self.address_translator = ClientRoutesAddressTranslator(self._client_routes_handler)

            if contact_points is _NOT_SET or not self._contact_points_explicit:
                seed_addrs = [ep.connection_addr for ep in client_routes_config.endpoints
                             if ep.connection_addr]
                if seed_addrs:
                    self.contact_points = seed_addrs
                    self._contact_points_explicit = True
                    log.info("[client routes] Using %d endpoint connection addresses as contact points",
                            len(seed_addrs))
        elif address_translator is not None:
    ...
    def shutdown(self):
        ...
        if self._client_routes_handler is not None:
            try:
                self._client_routes_handler.shutdown()
            except Exception:
                log.warning("Error shutting down client routes handler", exc_info=True)
...
class ControlConnection(object):
    ...
    def _try_connect(self, endpoint):
        ...
        try:
            ...
            if self._cluster._client_routes_handler is not None:
                try:
                    self._cluster._client_routes_handler.initialize(self)
    ...
    def _reconnect(self):
        ...
        try:
            ...
            # Notify client routes handler of reconnection (full re-read)
            if self._cluster._client_routes_handler is not None:
                try:
    ...
    def _handle_client_routes_change(self, event):
        """
        ...
        connection_ids = event.get("connection_ids", [])
        host_ids = event.get("host_ids", [])

        log.debug("[control connection] Received CLIENT_ROUTES_CHANGE: change_type=%s, "
                  "connection_ids=%s, host_ids=%s", change_type, connection_ids, host_ids)
