...
class ResultSet(object):
    ...
    def was_applied(self):
        ...
        # `isinstance` is checked first so it always short-circuits before
        # is_lwt() is looked up: BatchStatement is never eligible for the fast
        # path anyway, and the getattr/callable guard means a query object
        # that doesn't implement is_lwt() (e.g. a None query, or some other
        # non-Statement/duck-typed object) falls through to the slow path
        # below instead of raising AttributeError.
