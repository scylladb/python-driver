...
class VectorType(_CassandraType):
    ...
    def apply_parameters(cls, params, names):
        ...
        # Only ever consider this for subtypes that genuinely have a fixed
        # serialized size (subtype_ss is not None): the wire format for any
        # type without a fixed size is vint-length-prefixed per element (e.g.
        # smallint/tinyint on Cassandra 5.0, see ShortType/ByteType above), and
        # treating it as fixed-width would misparse real vector data.
        vector_struct = None
        numpy_dtype = None
        if subtype_ss is not None:
...
# NOTE: ShortType (smallint) and ByteType (tinyint) are intentionally excluded.
# Although they have a fixed in-memory representation, Cassandra 5.0 does not
# treat them as fixed-width for vector serialization (AbstractType.valueLengthIfFixed()
# defaults to variable-length, and neither ShortType.java nor ByteType.java override
# it) so their vector elements are vint-length-prefixed on the wire, same as any
# other variable-size type. Only include types here that have a real fixed
# serial_size(); apply_parameters() also gates on subtype.serial_size() is not
# None as a second line of defense.
