    ...
    def set(self, key, session):
        ...
        with self._lock:
            ...
            if len(self._sessions) >= self._max_size:
                # Prefer evicting an expired entry over the LRU live one so
                # that a still-valid session is not displaced by a stale one.
                # Scan from the least-recently-used end to evict the oldest
                # expired entry first; fall back to LRU eviction if none found.
                evicted = False
...
class Connection(object):
        ...
        if self._ssl_session_cache is not None:
            ...
            if cached_session is not None:
                try:
                    ...
                    log.debug("TLS session restore attempted for %s key=%s", self.endpoint, self._ssl_session_cache_key())
                except (AttributeError, ssl.SSLError, ValueError) as e:
                    log.debug("Could not restore TLS session for %s: %s", self.endpoint, e)
            else:
                log.debug("No cached TLS session found for %s key=%s", self.endpoint, self._ssl_session_cache_key())
    ...
    def _cache_tls_session_if_needed(self):
        ...
        When the TLS handshake was an abbreviated resumption (``session_reused``
        is ``True``), the session is already in the cache, so the write is
        skipped to avoid unnecessary lock contention.
        """
        if self._ssl_session_cache is not None and self.ssl_context is not None:
            # session_reused is a property on stdlib ssl.SSLSocket but a method
            # on PyOpenSSL SSL.Connection; call it if callable so that subclasses
            # that use PyOpenSSL without overriding this method still work.
            _session_reused = getattr(self._socket, 'session_reused', False)
            session_reused = _session_reused() if callable(_session_reused) else _session_reused
            session = getattr(self._socket, 'session', None)
            tls_version = getattr(self._socket, 'version', lambda: None)()
            log.debug("TLS cache check for %s: session_reused=%s session=%s tls_version=%s key=%s",
                      self.endpoint, session_reused, session, tls_version, self._ssl_session_cache_key())
            if session_reused:
                return
