        ...
        try:
            while not stop_event.is_set():
                ...
                if exhausted:
                    ...
                    # everything has caught up. Read ``_exhausted`` under
                    # ``condition`` rather than bare -- it is only ever
                    # written under this same lock (by this thread or by
                    # ``execute()``'s initial dispatch before this thread
                    # started), and reading it under the lock here removes
                    # any doubt about cross-thread visibility instead of
                    # relying on this thread being the sole writer.
                    with condition:
                        self._current += count
