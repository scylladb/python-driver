...
UTC_DATETIME_EPOC = datetime.datetime.fromtimestamp(
...
def min_uuid_from_time(timestamp):
    ...
    """
    return uuid_from_time(
        timestamp, 0x808080808080, 0x80
    )  # Cassandra does byte-wise comparison; fill with min signed bytes (0x80 = -128)
...
def max_uuid_from_time(timestamp):
    ...
    """
    return uuid_from_time(
        timestamp, 0x7F7F7F7F7F7F, 0x3F7F
    )  # Max signed bytes (0x7f = 127)
...
class OrderedMap(Mapping):
    ...
    def __delitem__(self, key):
        ...
        try:
            ...
            self._index = dict(
                (k, i if i < index else i - 1) for k, i in self._index.items()
            )
    ...
    def __eq__(self, other):
        ...
        try:
            d = dict(other)
            return len(d) == len(self._items) and all(
                i[1] == d[i[0]] for i in self._items
            )
        except KeyError:
            return False
        except TypeError:
...
class Time(object):
    ...
    def _from_timestring(self, s):
        try:
            ...
            self.nanosecond_time = (
                base_time.tm_hour * Time.HOUR
    ...
    def __eq__(self, other):
        ...
        return (
            self.nanosecond_time % Time.MICRO == 0
            and datetime.time(
                hour=self.hour,
                minute=self.minute,
                second=self.second,
...
class Point(object):
    ...
    def from_wkt(s):
        ...
        if geom["type"] != "Point":
            raise ValueError(
                "Invalid WKT geometry type. Expected 'Point', got '{0}': '{1}'".format(
                    geom["type"], s
                )
            )
...
class LineString(object):
    ...
    def from_wkt(s):
        ...
        if geom["type"] != "LineString":
            raise ValueError(
                "Invalid WKT geometry type. Expected 'LineString', got '{0}': '{1}'".format(
                    geom["type"], s
                )
            )
...
class Polygon(object):
    ...
    def __init__(self, exterior=tuple(), interiors=None):
        ...
        self.interiors = (
            tuple(_LinearRing(e) for e in interiors) if interiors else tuple()
        )
    ...
    def __str__(self):
        ...
        rings = [
            "(%s)" % ", ".join("%r %r" % (x, y) for x, y in ring) for ring in rings
        ]
    ...
    def __repr__(self):
        return "%s(%r, %r)" % (
            ...
            [ring.coords for ring in self.interiors],
        )
    ...
    def from_wkt(s):
        ...
        if geom["type"] != "Polygon":
            raise ValueError(
                "Invalid WKT geometry type. Expected 'Polygon', got '{0}': '{1}'".format(
                    geom["type"], s
                )
            )
...
class DateRangePrecision(object):
    ...
    def round_up_to_precision(cls, ms, precision):
        ...
        if precision == cls.MONTH:
            ...
            upper_date = datetime.datetime.max.replace(
                year=date_ms.year,
                month=date_ms.month,
    ...
    def validate(self):
        ...
        if None in attrs:
            raise TypeError(
                ...
                % (self.__class__.__name__, self.__class__.__name__, self)
            )
...
class DateRange(object):
    ...
    def __init__(self, lower_bound=None, upper_bound=None, value=None):
        ...
        lower_bound = (
            DateRangeBound.from_value(lower_bound).round_down()
            if lower_bound
            else lower_bound
        )
        upper_bound = (
            DateRangeBound.from_value(upper_bound).round_up()
            if upper_bound
            else upper_bound
        )
    ...
    def __lt__(self, other):
        return (str(self.lower_bound), str(self.upper_bound), str(self.value)) < (
            str(other.lower_bound),
            str(other.upper_bound),
            str(other.value),
        )
...
class Version(object):
    ...
    TODO: consider replacing with packaging.version.
    """
    ...
    def __init__(self, version):
        ...
        if len(parts) > 4:
            ...
            log.warning(
                "Unrecognized version: {}. Only 4 components plus prerelease are supported. "
                "Assuming version as {}{}".format(
                    version, ".".join(parts[:-5:-1]), prerelease_string
                )
            )
        ...
        try:
            ...
            raise ValueError(
                "Couldn't parse version {}. Version should start with a number".format(
                    version
                )
        ...
        try:
            ...
            log.warning(
                "Unrecognized version {}. Assuming version as {}".format(
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self._compare_version_part(self.build, other.build, lambda s, o: s == o)
            and self._compare_version_part(
    ...
    def __gt__(self, other):
        ...
        if self.prerelease and not other.prerelease:
            ...
            is_prerelease_gt = self._compare_version_part(
                self.prerelease, other.prerelease, lambda s, o: s > o
            )

        return (
            self.major > other.major
            or (is_major_ge and self.minor > other.minor)
            or (is_major_ge and is_minor_ge and self.patch > other.patch)
            or (is_major_ge and is_minor_ge and is_patch_ge and is_build_gt)
            or (
                is_major_ge
                and is_minor_ge
                and is_patch_ge
...
def maybe_add_timeout_to_query(
    stmt: str, metadata_request_timeout: Optional[datetime.timedelta]
) -> str:
