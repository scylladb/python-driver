...
class Statement(object):
    ...
    def routing_token(self, token_class):
        ...
        partitioner (``token_class``), both stable for a given statement, so the
        (Murmur3) hash is memoized. A single request would otherwise recompute it
        three times -- in the load balancing policy, when selecting the target
        shard, and when building the tablet_version_block -- so caching keeps that
        hot path to one hash. Returns ``None`` when there is no routing key.
        """
