...
from cassandra.util import add_timeout_to_query
...
class _SchemaParser(object):
    ...
    def _query_build_rows(self, query_string, build_func):
        query = SchemaQueryMessage(query=query_string, consistency_level=ConsistencyLevel.ONE,
                                   fetch_size=self.fetch_size, custom_timeout=self.metadata_request_timeout)
...
class SchemaParserV22(_SchemaParser):
    ...
    def get_table(self, keyspaces, keyspace, table):
        ...
        cf_query = SchemaQueryMessage(
            query=self._SELECT_COLUMN_FAMILIES + where_clause,
            consistency_level=cl,
            custom_timeout=self.metadata_request_timeout,
        )
        col_query = SchemaQueryMessage(
            query=self._SELECT_COLUMNS + where_clause,
            consistency_level=cl,
            custom_timeout=self.metadata_request_timeout,
        )
        triggers_query = SchemaQueryMessage(
            query=self._SELECT_TRIGGERS + where_clause,
            consistency_level=cl,
            custom_timeout=self.metadata_request_timeout,
        )
    ...
    def _query_all(self):
        ...
        queries = [
            SchemaQueryMessage(
                query=self._SELECT_KEYSPACES,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            ),
            SchemaQueryMessage(
                query=self._SELECT_COLUMN_FAMILIES,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            ),
            SchemaQueryMessage(
            ...
            SchemaQueryMessage(
                query=self._SELECT_TYPES,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            ),
            SchemaQueryMessage(
                query=self._SELECT_FUNCTIONS,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            ),
            SchemaQueryMessage(
                query=self._SELECT_AGGREGATES,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            ),
            SchemaQueryMessage(
                query=self._SELECT_TRIGGERS,
                consistency_level=cl,
                custom_timeout=self.metadata_request_timeout,
            )
        ]
...
class SchemaParserV3(SchemaParserV22):
    ...
    def get_table(self, keyspaces, keyspace, table):
        ...
        where_clause = bind_params(" WHERE keyspace_name = %%s AND %s = %%s" % (self._table_name_col), (keyspace, table), _encoder)
        cf_query = SchemaQueryMessage(
            query=self._SELECT_TABLES + where_clause,
            consistency_level=cl, fetch_size=fetch_size, custom_timeout=self.metadata_request_timeout)
        col_query = SchemaQueryMessage(
            query=self._SELECT_COLUMNS + where_clause,
            consistency_level=cl, fetch_size=fetch_size, custom_timeout=self.metadata_request_timeout)
        indexes_query = SchemaQueryMessage(
            query=self._SELECT_INDEXES + where_clause,
            consistency_level=cl, fetch_size=fetch_size, custom_timeout=self.metadata_request_timeout)
        triggers_query = SchemaQueryMessage(
            query=self._SELECT_TRIGGERS + where_clause,
            consistency_level=cl, fetch_size=fetch_size, custom_timeout=self.metadata_request_timeout)
        scylla_query = SchemaQueryMessage(
            query=self._SELECT_SCYLLA + where_clause,
            consistency_level=cl, fetch_size=fetch_size, custom_timeout=self.metadata_request_timeout)
        ...
        view_query = SchemaQueryMessage(
            query=self._SELECT_VIEWS + where_clause,
            consistency_level=cl,
    ...
    def _query_all(self):
        ...
        fetch_size = self.fetch_size
        queries = [
            SchemaQueryMessage(query=self._SELECT_KEYSPACES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TABLES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_COLUMNS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TYPES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_FUNCTIONS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_AGGREGATES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TRIGGERS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_INDEXES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIEWS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_SCYLLA, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
        ]
    ...
    def _query_all(self):
        ...
        queries = [
            ...
            SchemaQueryMessage(query=self._SELECT_VIEWS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            # V4-only queries
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_KEYSPACES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_TABLES, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_COLUMNS, fetch_size=fetch_size, consistency_level=cl,
                               custom_timeout=self.metadata_request_timeout)
        ]
...
class SchemaParserDSE68(SchemaParserDSE67):
    ...
    def get_table(self, keyspaces, keyspace, table):
        ...
        cl = ConsistencyLevel.ONE
        where_clause = bind_params(" WHERE keyspace_name = %%s AND %s = %%s" % (self._table_name_col), (keyspace, table), _encoder)
        vertices_query = SchemaQueryMessage(
            query=self._SELECT_VERTICES + where_clause,
            consistency_level=cl,
            custom_timeout=self.metadata_request_timeout,
        )
        edges_query = SchemaQueryMessage(
            query=self._SELECT_EDGES + where_clause,
            consistency_level=cl,
            custom_timeout=self.metadata_request_timeout,
        )
    ...
    def _query_all(self):
        ...
        queries = [
            # copied from v4
            SchemaQueryMessage(query=self._SELECT_KEYSPACES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TABLES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_COLUMNS, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TYPES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_FUNCTIONS, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_AGGREGATES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_TRIGGERS, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_INDEXES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIEWS, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_KEYSPACES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_TABLES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_VIRTUAL_COLUMNS, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            # dse6.8 only
            SchemaQueryMessage(query=self._SELECT_VERTICES, consistency_level=cl, custom_timeout=self.metadata_request_timeout),
            SchemaQueryMessage(query=self._SELECT_EDGES, consistency_level=cl, custom_timeout=self.metadata_request_timeout)
        ]
...
def get_schema_parser(connection, server_version, dse_version, timeout, fetch_size=None, metadata_request_timeout=None):
