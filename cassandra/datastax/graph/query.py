...
_graph_options = (
    ...
    (
        "graph_protocol",
        'the graph protocol that the server should use for query results (default "graphson-1-0")',
        "graph-results",
    ),
    (
        "graph_read_consistency_level",
        """read `cassandra.ConsistencyLevel <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/#cassandra.ConsistencyLevel>`_ for graph queries (if distinct from session default).
Setting this overrides the native `Statement.consistency_level <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/query/#cassandra.query.Statement.consistency_level>`_ for read operations from Cassandra persistence""",
        "graph-read-consistency",
    ),
    (
        "graph_write_consistency_level",
        """write `cassandra.ConsistencyLevel <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/#cassandra.ConsistencyLevel>`_ for graph queries (if distinct from session default).
Setting this overrides the native `Statement.consistency_level <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/query/#cassandra.query.Statement.consistency_level>`_ for write operations to Cassandra persistence.""",
    ...
    def __init__(self, **kwargs):
        ...
        for attr, value in kwargs.items():
            if attr not in _graph_option_names:
                warn(
                    "Unknown keyword argument received for GraphOptions: {0}".format(
                        attr
                    )
                )
...
class Result(object):
    ...
    def as_vertex(self):
        ...
        try:
            return Vertex(
                self.id, self.label, self.type, self.value.get("properties", {})
            )
