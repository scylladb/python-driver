...
cdef inline num_t unpack_num(Buffer *buf, num_t *dummy=NULL): # dummy pointer because cython wants the fused type as an arg
    ...
    cdef float ftemp      # For float byte-swapping (memcpy-based bit-cast)
    ...
    if num_t is int16_t or num_t is uint16_t:
        ...
        # Use memcpy-based bit-casts rather than pointer-cast dereferences
        # (e.g. (<uint32_t*>&ret)[0] / (<float*>&temp32)[0]): reinterpreting
        # a uint32_t's bits as a float (or vice versa) through a raw
        # pointer-cast violates C's strict-aliasing rules and can be
        # miscompiled under optimization/LTO. memcpy is the well-defined way
        # to reinterpret a bit pattern across unrelated types.
        memcpy(&temp32, &ret, sizeof(uint32_t))
        temp32 = ntohl(temp32)
        memcpy(&ftemp, &temp32, sizeof(float))
        return ftemp
    else:
