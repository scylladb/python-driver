...
from cassandra.protocol import (
    write_int, write_short, write_string, write_value,
    ProtocolHandler, _ProtocolHandler, HAVE_CYTHON,
)
...
SCENARIOS = {
    ...
    "medium_1k_256B":   ("1000 rows, 3 cols (text 256B, int, bigint)",      _scenario_medium_1k_256B),
    "medium_1k_1KB":    ("1000 rows, 3 cols (text 1024B, int, bigint)",     _scenario_medium_1k_1KB),
    "large_5k_1KB":     ("5000 rows, 3 cols (text 1024B, int, bigint)",     _scenario_large_5k_1KB),
    "large_1k_4KB":     ("1000 rows, 3 cols (text 4096B, int, bigint)",     _scenario_large_1k_4KB),
    "wide_5k_doubles":  ("5000 rows, 10 cols (10x double)",                 _scenario_wide_5k_doubles),
    "wide_1k_20cols":   ("1000 rows, 20 cols (10x text64, 5x int, ...)",    _scenario_wide_1k_20cols),
...
def _report(label, times, body_size, row_count):
    ...
    print("  %-14s  min=%s  median=%s  mean=%s  (%s, %s)" % (
        label,
        _format_time(t_min),
        _format_time(t_med),
        _format_time(t_mean),
...
def run_benchmark(scenarios, iterations, warmup, cython_only, python_only, cprofile_scenario):
    """
    ...
    if not handlers:
        print("ERROR: no handlers selected (Cython not available and --cython-only set)")
        sys.exit(1)
    ...
    for name in scenarios:
        ...
        print("Scenario: %s  (%s, %s body)" % (
            name, desc, _format_size(body_size)))
...
def main():
    ...
    parser.add_argument(
        "--cython-only", action="store_true", default=False,
        help="Only benchmark the Cython (fast) path")
    parser.add_argument(
        "--python-only", action="store_true", default=False,
        help="Only benchmark the pure-Python path")
    parser.add_argument(
        "--cprofile", type=str, default=None, metavar="SCENARIO",
    ...
    if args.cprofile and args.cprofile not in selected:
        parser.error("--cprofile scenario '%s' is not in the selected scenarios" % args.cprofile)
