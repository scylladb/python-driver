...
Run with: pytest benchmarks/test_utf8_decode_benchmark.py -v --benchmark-sort=name
...
class TestCorrectness:
    """Verify that the optimized decode produces correct results."""
    ...
    def test_utf8_ascii_only(self):
        ...
        reader = BytesIOReader(buf)
        rows = parser.parse_rows(reader, desc)
        assert rows[0][0] == "Hello, World! 12345"
    ...
    def test_utf8_multibyte(self):
        ...
        reader = BytesIOReader(buf)
        rows = parser.parse_rows(reader, desc)
        assert rows[0][0] == "Héllo wörld! こんにちは 🌍"
    ...
    def test_ascii_basic(self):
        ...
        reader = BytesIOReader(buf)
        rows = parser.parse_rows(reader, desc)
        assert rows[0][0] == "Simple ASCII text 12345 !@#"
