...
def bench_write_value_constants():
    ...
    # number=200_000, repeat=7: matches the methodology used for the
    # prerequisite PR's benchmark (best-of-repeat via min()).
    n = 200_000
    repeats = 7
    t_precomputed = min(timeit.repeat(run_precomputed, number=n, repeat=repeats))
    t_pack = min(timeit.repeat(run_pack_each_time, number=n, repeat=repeats))

    print(f"Pre-computed constants ({n} iters, best of {repeats}, {len(params)} params): "
          f"{t_precomputed:.3f}s  ({t_precomputed / n * 1e6:.2f} us/call)")
    print(f"Pack each time         ({n} iters, best of {repeats}, {len(params)} params): "
