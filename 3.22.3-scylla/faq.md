...
See also [cqlengine FAQ](cqlengine/faq.md)
...
By default, retries are managed by the `Cluster.default_retry_policy` set on the session Cluster. It can also
be specialized per statement by setting `Statement.retry_policy`.

Retries are presently attempted on the same coordinator, but this may change in the future.

Please see [`policies.RetryPolicy`](api/cassandra/policies.md#cassandra.policies.RetryPolicy) for further details.
