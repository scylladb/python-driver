...
`Session.execute_graph()` or `Session.execute_graph_async()` for
...
See [Getting Started](getting_started.md) and [Execution Profiles](execution_profiles.md)
...
To see a more graph examples, see [DataStax Graph Examples](https://github.com/datastax/graph-examples/).
...
By default (with `GraphExecutionProfile.row_factory` set to `datastax.graph.graph_object_row_factory()`), known graph result
types are unpacked and returned as specialized types (`Vertex`, `Edge`). If the result is not one of these
types, a `datastax.graph.Result` is returned, containing the graph result parsed from JSON and removed from its outer dict.
...
You can use a different row factory by setting `cluster.ExecutionProfile.row_factory` or passing it to
`cluster.Session.execute_graph()`. For example, `datastax.graph.single_object_row_factory()` returns the JSON result string,
unparsed. `datastax.graph.graph_result_row_factory()` returns parsed, but unmodified results (such that all metadata is retained,
unlike `datastax.graph.graph_object_row_factory()`, which sheds some as attributes and properties are unpacked). These results
also provide convenience methods for converting to known types (`datastax.graph.Result.as_vertex()`, `datastax.graph.Result.as_edge()`,

> `datastax.graph.Result.as_path()`).

Vertex and Edge properties are never unpacked since their types are unknown. If you know your graph schema and want to
deserialize properties, use the `datastax.graph.GraphSON1Deserializer`. It provides convenient methods to deserialize by types (e.g.
...
All python types listed in [Graph Types]() can be passed as named parameters and will be serialized
