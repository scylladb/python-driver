...
`auth_provider` to an instance of a subclass
of [`AuthProvider`](api/cassandra/auth.md#cassandra.auth.AuthProvider).  When working
with Cassandra’s `PasswordAuthenticator`, you can use
the [`PlainTextAuthProvider`](api/cassandra/auth.md#cassandra.auth.PlainTextAuthProvider) class.
...
[`SaslAuthProvider`](api/cassandra/auth.md#cassandra.auth.SaslAuthProvider) is provided for generic SASL authentication mechanisms,
utilizing the `pure-sasl` package.
If these do not suit your needs, you may need to create your own subclasses of
[`AuthProvider`](api/cassandra/auth.md#cassandra.auth.AuthProvider) and [`Authenticator`](api/cassandra/auth.md#cassandra.auth.Authenticator).  You can use the Sasl classes
as example implementations.

### Protocol v1 Authentication

When working with Cassandra 1.2 (or a higher version with
`protocol_version` set to `1`), you will not pass in
an [`AuthProvider`](api/cassandra/auth.md#cassandra.auth.AuthProvider) instance.  Instead, you should pass in a
...
To enable SSL with version 3.17.0 and higher, you will need to set `Cluster.ssl_context` to a
`ssl.SSLContext` instance to enable SSL. Optionally, you can also set `Cluster.ssl_options`
...
as described in the following examples or implement your own `EndPoint` and
`EndPointFactory`.

The following examples assume you have generated your Cassandra certificate and
keystore files with these intructions:

* [Setup SSL Cert](https://docs.datastax.com/en/dse/6.7/dse-admin/datastax_enterprise/security/secSetUpSSLCert.html)

It might be also useful to learn about the different levels of identity verification to understand the examples:

* [Using SSL in DSE drivers](https://docs.datastax.com/en/dse/6.7/dse-dev/datastax_enterprise/appDevGuide/sslDrivers.html)
...
[`TwistedConnection`](api/cassandra/io/twistedreactor.md#cassandra.io.twistedreactor.TwistedConnection) or `EventletConnection`, you must pass a
...
To enable SSL you will need to set `Cluster.ssl_options` to a
...
the [python ssl documentation](https://docs.python.org/2/library/ssl.html#ssl.wrap_socket) for
your configuration. For further reading, Andrew Mussey has published a thorough guide on
[Using SSL with the DataStax Python driver](http://blog.amussey.com/post/64036730812/cassandra-2-0-client-server-ssl-with-datastax-python).
...
Implementations are `DSEPlainTextAuthProvider`, `DSEGSSAPIAuthProvider` and [`SaslAuthProvider`](api/cassandra/auth.md#cassandra.auth.SaslAuthProvider).
...
Please see the [official documentation](https://docs.datastax.com/en/latest-dse/datastax_enterprise/unifiedAuth/unifiedAuthTOC.html) for more details on the feature and configuration process.
