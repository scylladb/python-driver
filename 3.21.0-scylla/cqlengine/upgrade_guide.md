...
Going forward, the `Date` type represents a `date` for Cassandra 2.2+
([PYTHON-245](https://datastax-oss.atlassian.net/browse/PYTHON-245)).
Users of the legacy functionality should convert models to use `DateTime` (which
...
[`cassandra.ConsistencyLevel`](../api/cassandra.md#cassandra.ConsistencyLevel) remains.
...
This inititializer parameter is now deprecated. Applications should use `Double` for CQL `double`, and `Float`
for CQL `float`.
...
`cassandra.cqlengine.management.delete_keyspace` is deprecated in favor of a new function, `drop_keyspace()`. The
...
- Replace the ‘_\_polymorphic_key_\_’ class attribute the derived classes with `__discriminator_value__`
